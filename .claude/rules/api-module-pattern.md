...
1. **`packages/shared/src/schemas/<module>.ts`** — schemi Zod (Create/Update/Filter/Response). Esporta da `packages/shared/src/index.ts` **e** aggiungi il subpath export in `packages/shared/package.json` → `./schemas/<module>`.
   **Il criterio, scritto una volta (S1101, 2026-09-14 — prima ognuno decideva da sé):** nel contratto condiviso sta **tutto ciò che attraversa il confine e ha una forma che un consumatore deve conoscere** — body, querystring, response, e i parametri di percorso **composti o vincolati** (`{ id, number }`, `code` con lunghezza) o **riusati da più rotte**. Sta **sul posto** in `routes.ts` solo l'identificativo banale di una rotta sola (`params: z.object({ id: z.uuid() })`): lì il contratto non aggiunge informazione. Misurato: 93 moduli su 96 già così; `compensation` ed `evidence` sono le due eccezioni dichiarate (un uuid, una rotta), `dashboard` e `provenance` allineati.
   **La risposta si dichiara SEMPRE**, anche quando non è JSON — il listino OpenAPI (R6, `API_DOCS_ENABLED`) la legge da lì, e una rotta senza `response` vi compare come «200 Default Response» anche se risponde 204 o un file. Le forme: `response: { 204: z.null() }` con `reply.code(204).send(null)` per le cancellazioni; `response: { 200: { description, content: { "<mime>": { schema: z.string() } } } }` per file e flussi (Fastify non serializza stringhe, buffer e stream: lo schema documenta, non trasforma — `z.unknown()` se il tipo del provider deve accettare uno stream in `send`). Esempi vivi: `analytics/routes.ts` (export), `me/routes.ts` (SSE e media), `visualization-exports/routes.ts`.
...
4. **`apps/api/src/modules/<module>/routes.ts`** — `FastifyPluginAsyncZod` con `requirePermission('<resource>:<verb>')` su ogni rotta più `app.verifyCsrf` su POST/PATCH/DELETE. Gli errori lanciati da service/repository usano le classi tipizzate in `src/errors/index.ts` (`UnauthorizedError`, `ForbiddenError`, `NotFoundError`, `ValidationError`, `ConflictError`) con un codice `SCREAMING_SNAKE` come secondo argomento. L'error handler lo trasforma in un body stabile `{error:{code, message}}` (`details?` per gli errori di validazione); il request id torna nell'header di risposta `x-request-id`, non nel body.

**Due livelli di diniego, due codici diversi — non confonderli** (misurato 2026-08-08 su 532 usi di `requirePermission`):

| Chi nega | Quando | Codice |
|---|---|---|
| `requirePermission` (middleware) | il ruolo non ha quel permesso | **`FORBIDDEN`** — è il default, e vale su **520 rotte su 532** |
| `requirePermission(code, deniedCode)` | un modulo vuole un codice proprio nel contratto pubblico | il `deniedCode` passato — oggi 12 rotte, tutte `*_ADMIN_ONLY` |
| il **service** | il permesso c'è ma lo scope no: non è il tuo passo, è un altro tenant, il ruolo non basta per quella relazione | **`PERMISSION_DENIED`** (`approvals`, `teams`, `mfa-policy`, `semantic-matching`) |

Un test che si aspetta `PERMISSION_DENIED` da un permesso mancante **fallisce**: quel caso risponde `FORBIDDEN`. Codici esistenti da imitare: `LOGIN_INVALID`, `REFRESH_REPLAY_DETECTED`, `RBAC_NOT_LOADED`.
