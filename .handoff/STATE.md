...
3. **A2 follow-up** (decide Enzo per-finding, `FINDINGS/WS-H.md`): 🔴 **D-28** verificare `TRUST_PROXY=true` nel `.env` PROD (rate-limit per-IP); 6 quick-win QW-H1..6 (drizzle dead-dep→chiude alert esbuild · media magic-byte sniff · rate-limit keyGen · requirePermission skill-taxonomy · mailer redaction · matching/reindex limit). + **S-100X-A3** (audit successivo).
...
- **D-28 / TRUST_PROXY**: il `.env` PROD ha `TRUST_PROXY=true`? (non verificabile da qui, secret hygiene). Se no → rate-limit per-IP degradato dietro nginx.
