...
3. **vm-deploy → PROD** (opzionale) — portare next 16 + vista overtime in produzione via `scripts/vm-deploy.sh` (PROD ancora a `36676f0`, pre-S969).

## Open questions

- `VOYAGE_API_KEY` nel `.env` VM → sblocca ② AI P1 (azione Enzo).
- Le 4 WHAT-decisions sopra: quale direzione aprire?
- next 16 in PROD: deploy ora o attendere un driver?
