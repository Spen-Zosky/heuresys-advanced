...
0. **[BLOCCANTE — D-74] Chiudere la regressione dell'unique index skill + deploy i18n**. In S1024 il dedup (000189) ha aggiunto `sys_skills_natural_key_uq` che ROMPE l'INSERT di skill non-idempotente (tenant-materialization + reconciliation) in CI e PROD. **De-risk già fatto**: index DROPpato da PROD+heuresys_ci (mig **000194**). Da fare: (a) `ON CONFLICT DO NOTHING` nei repository che inseriscono skill, poi ripristinare l'index in sicurezza; (b) rendere **data-derived** gli anchor hardcoded di `analytics.integration.test.ts` (rotti dai dati resi reali — skill 14041, possesso/gruppi); (c) far tornare verde la CI (heuresys_ci è aggiornato a 000194); (d) **deploy del wiring API i18n** (codice già su main a `43885f6d`, atteso dal ci-gate — non ancora live in PROD; il DB PROD è invece già completo). Lezione: dopo un cambio strutturale (constraint/index) rilanciare la SUITE INTEGRATION COMPLETA prima del commit, non solo twice-run/validate.
