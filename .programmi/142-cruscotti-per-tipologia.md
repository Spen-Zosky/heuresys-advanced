...
- [ ] **F3b — I dati dentro le viste** — ogni vista `open` deve restituire il proprio contenuto reale; le `masked` dichiarano il perché senza i valori. Qui la granularità smette di essere un modello e diventa una query · budget ~140k
- [ ] **F4 — Frontend + dimostrazione live per tipologia** — pagine, e un login reale **per ogni** tipologia (non una a campione) · budget ~250k

## Da dove si riprende

**F3b — I dati dentro le viste.** F3a ha consegnato il catalogo il 2026-08-16: le rotte
esistono, il permesso per famiglia è applicato, e ogni vista esce già con la propria
**modalità** (`open`/`masked`/`denied`). Resta il contenuto.
