...
- [x] **F4 — Frontend + dimostrazione live per tipologia** — FATTO 2026-08-19 · **UNA pagina per otto famiglie** (`/dashboard/[famiglia]`), non otto gemelle: la famiglia si trova confrontando la route col catalogo, quindi la corrispondenza fra `/dashboard/azienda` e `company` resta nel database e non in una mappa di frontend. Un componente per **forma** (`counters`/`series`/`list`), non per vista. Mig. `000326`: 7 voci di menu, aggancio **per route**, attivazione delle otto famiglie, e le classi di dato **ereditate dalle viste** invece che riscritte. E2E `dashboard-famiglie.spec.ts` — **19 passati, 1 skipped, zero flaky senza retry**, con **sei login reali** (amministratore di piattaforma, di azienda, manager, capo filiale, persona senza deleghe, custode): ogni famiglia che il catalogo offre a ciascuno è raggiungibile e disegna le sue viste, e una famiglia non offerta risponde «non disponibile» senza rivelare se non esiste o non è sua.
      🔬 **La prova generale ha fermato DUE difetti che sarebbero stati CI rossa 25 minuti dopo il push**, entrambi alla seconda passata e nessuno dei due nella migrazione nuova: (1) la `000316` asseriva «nessuna famiglia è attiva» — una **fotografia del momento scritta nella forma di un invariante**, destinata a diventare falsa esattamente quando F4 fosse stata fatta; emendata nell'invariante che voleva dire, «nessuna famiglia attiva SENZA pagina». (2) la `000317` teneva il **totale esatto** delle dichiarazioni di classe (26) e nel proprio commento prescriveva il rimedio: «chi aggiungerà righe dopo di me deve spostare QUESTO conteggio nel proprio file». Fatto: lì resta `>= 26`, il totale esatto (41) vive ora nella `000326`.
      🔬 **Un flaky visto e spiegato, non insabbiato**: al primo giro l'E2E ha segnato 1 flaky su `platformAdmin`. Rieseguito senza retry, due volte, zero flaky: era il cold-start della compilazione della rotta dinamica nel dev server, non il prodotto · budget ~250k

## Chiusa il 2026-08-19 — le tre condizioni, verificate una per una

| la chiusura dichiarata chiedeva | come è stata verificata |
|---|---|
| ogni cruscotto ha un permesso proprio | 7 permessi di famiglia + `self` senza, che è I17: il Self-Service non si concede, si garantisce |
| nessuna pagina è raggiungibile da chi non può vederne il contenuto | E2E: per ogni persona, una famiglia fuori dal suo catalogo risponde «non disponibile» — e il servizio nega con `FORBIDDEN`, lo stesso codice di `requirePermission` |
| la granularità è dichiarata per vista e verificata con un login reale per tipologia | **sei** login reali, non uno a campione; e sulla stessa vista economica il mandato tecnico legge `masked` mentre quello HR legge i valori |

## Da dove si riprendeva (storico)
