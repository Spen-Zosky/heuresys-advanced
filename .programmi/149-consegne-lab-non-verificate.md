...
- [ ] **F4 La prossima consegna che arriva, o la prossima ingerita che qualcuno cita** — budget ~30k a consegna · **misurata 2026-09-13 (S1099)**: `lab_inbox.py` → vuota; `check_verifica_consegne.py` VERDE, 6/6 consegne citate con marker (nessuna NON-VERIFICATO): oggi la fase non scatta · **eseguita 2026-09-13 (S1098) sulla consegna P4** (`2026-08-16-tenant-builder-p4-estrattori-e-ingestori`, citata da `#206` ripresa da Enzo): verificata **eseguendola**, non leggendola — sette passaggi corretti sul DB vivo (corsa in `sys_seed_acquisition_runs`, impronta non unica, mappa degli esiti, posizione vacante come caso normale, registro già esistente, una-posizione-una-persona, precondizione alla firma); marker aggiornato da CONFERMATO (S1096) a **PARZIALE · S1098**; `check_verifica_consegne.py` VERDE exit 0; inbox vuota (`lab_inbox.py` + `ls`: sola `ingerite/`, 24 agosto). La fase resta aperta perché continuativa · eseguita 2026-09-12 (S1095) su `2026-08-12-guardia-psql-opzioni-raggruppate` (citata due volte dal register, marker NON-VERIFICATO da S1094): cinque affermazioni portanti misurate una per una → esito **PARZIALE**, scritto nel file — il difetto e' confermato ma la consegna e' gia' stata eseguita (patch nel repo, `PSQL_OPZIONI_CON_VALORE` riga 772; tre casi in `_cases()`; `hook.sh selftest` 122/0; `#121` DONE), quindi meta' del documento descrive uno stato che non esiste piu'. `check_verifica_consegne.py` la legge: `esito PARZIALE · S1095`. Restano 4 consegne citate con marker NON-VERIFICATO (istruzione-vincolante, e22-e23, p3, p4): la fase resta aperta per costruzione
