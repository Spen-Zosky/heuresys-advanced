...
- [ ] **T9b La costruzione vera in produzione — TENTATA il 2026-08-19, esito DISFATTO (E28)** — 2026-08-19 · **11/11 verdi** più PROVA C 3/3, e poi l'azienda è stata **rimossa per intero su decisione di Enzo**. Il motore ha funzionato end-to-end su dati veri: 184 righe costruite e **184 nel registro** (PROVA A), `apply` prima dell'approvazione non costruisce (PROVA B, HTTP 409), il fascicolo è `APPLIED` con la data, archiviare non cancella (PROVA C). Copertura del metro **11/144 tabelle (7,6%) · 29/329 relazioni (8,8%)** — identica al gemello, e il piano ordina di non farla salire. Controllo incrociato: **165 righe su 184 invisibili al marchio**, il registro le conosce tutte.
      ⚠⚠ **MA L'AZIENDA ERA UNA TERZA BANCA, ed è il difetto che conta più della prova.** Enzo lo ha visto subito: *«ogni volta crei una banca e questo è un errore grossolano»*. Misurato: nel database esiste **una sola** famiglia di blueprint (`FIN_BANKING`) e **una sola** variante (`Regional Retail Bank (Medium)`), e l'archetipo è **cablato in TypeScript** — `apps/api/src/modules/tenant-materialization/blueprints.ts`, 296 righe con «Retail Bank — reference org skeleton», «Credit Risk Analysis», «Digital Banking», e un commento che dice di ricalcare «come RTL Bank lo fa DAVVERO». Sono esattamente i cinque simboli che **`#132` F3** elencava da ritirare — e **F3 È STATA FATTA il 2026-08-19 (S1072)**: il file non esiste più, i cinque simboli nemmeno, e un cancello nei test lo verifica a ogni corsa. ⚠ Conseguenza diretta per **T9b**: la costruzione in produzione **non è rifacibile finché `#132` F6 non avrà riempito un modello** — la versione ancorata dichiara ora `BLUEPRINT_CONTENT` e quelle tabelle sono vuote, quindi l'atto si rifiuta con `BLUEPRINT_CONTENT_EMPTY` invece di costruire una quarta banca. Non è un blocco nuovo: è la stessa premessa che il piano di `#132` dichiara da sempre («da F3 a F6 nessuna azienda è costruibile»), ora visibile anche da qui.
      🔬 **La mia richiesta di conferma è stata incompleta**: ho chiesto a Enzo di autorizzare «una terza azienda vera» senza dirgli che sarebbe stata **necessariamente una banca**. Quella era l'informazione che serviva per decidere.
