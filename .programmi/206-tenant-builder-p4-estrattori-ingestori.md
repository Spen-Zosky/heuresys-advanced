...
- [ ] **T9 La corsa vera su un'azienda vera (E20)** — ⛔ **FUORI DAL CONFINE DI QUESTA SESSIONE**: pretende `#198` T9b (azienda vera in produzione), che aspetta un modello non bancario dalla ricerca. ✅ **PROVA SUL GEMELLO FATTA 2026-09-13 (E27)**: `prova-live-206.mts` **17/17 verdi** contro il DB del linux-pc (API su `:3011` puntata a `:5434`, login reale `enzo.spenuso@heuresys.com`): azienda `P4PROVA202609130228` costruita da P3 (modello di prova seminato, E29), 3 righe → 1 AMMESSA · 1 CIECA · 1 esclusa, firma dell'amministratore dell'azienda, 2 persone nate, 2 posizioni + 2 unità `CONFIRMED`, 0 `SUPERSEDED`, fonte `INGESTED`, seconda corsa 409. Referto in `.zp/prove/206-prova-live-gemello-20260913.txt`. ⚠ **Scoperta**: l'unico modello con contenuto in produzione (`REGIONAL_RETAIL_BANK_MEDIUM`) **non è costruibile** — `BLUEPRINT_CONTENT_INCOHERENT`, 132 competenze ESCO senza categoria — quindi `#198` T9b è bloccata due volte (banca, e incoerente); l'azienda di prova resta sul gemello, che si rinfresca dal clone
...
- **`REGIONAL_RETAIL_BANK_MEDIUM` v1 (l'unico modello con contenuto) NON è costruibile**: 132 competenze `ESCO::…` senza categoria → `BLUEPRINT_CONTENT_INCOHERENT` all'atto. È un difetto del contenuto generato da `#132`/`#205`, non di P4: va a `#198`/`#205`. Misurato 2026-09-13 sul gemello.
