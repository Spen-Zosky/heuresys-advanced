...
- [ ] **F2 Gli 80 casi che non vengono eseguiti** — budget ~40k
      70 `skipped` + 1 `did not run` nella fase 3, 6 + 3 nelle altre. **Non** è `maxFailures`
      (nessuna config lo imposta, verificato) e **non** sono i blocchi `serial` (i tre file che
      li usano hanno 11 casi in tutto). La causa non è isolata: oggi uno strumento la *dichiara*
      a ogni corsa invece di sommare i verdi, ed è tutto ciò che è cambiato.
