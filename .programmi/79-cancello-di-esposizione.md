...
- [ ] **F3 Il prossimo lavoro che popola tabelle** — budget ~5k per esecuzione · eseguita 2026-09-13 (S1098) su `#206` T1-T8 (il modulo `tenant-import-runs` popola `sys_users`, `sys_user_position_assignments`, `sys_user_skills`, `sys_seed_*` e la nuova `staging.tenant_import_people`): `check_exposure.py` → **0 lacune**, exit 0 — le tabelle `sys.*` toccate erano già esposte, la tabella di atterraggio è `staging` (fuori dal perimetro del cancello per costruzione: non è un dato di prodotto, è l'ingresso) · eseguita 2026-09-12 (S1095) su `#54` F4: `check_exposure.py` → 73 scritte, 73 lette, **0 lacune**, exit 0. Resta aperta per costruzione: si ri-esegue al prossimo lavoro che popola tabelle
