...
| R-0 | SOSPESA | S1104 | 2026-09-17T01:05+02:00 | 38 |  | 000421 (applicata in produzione) | migrazione 000421 (VM, exit=0); esiti/R-0_porte.md, esiti/R-0.md; apps/api/src/lib/actor.ts (perimetroClienti/puoVedereCliente); test/unit/perimetro-clienti.unit.test.ts (6/6); modulo platform-tenant-assignments (3 rotte, 4/4 test verificati a mano — S-2 li pretendeva, mancavano); baseline S-5 aggiornata (93 file); DUE difetti trovati e corretti dalla prova generale stessa: z.coerce.boolean() → queryBoolean() (#209), permesso senza rotta (S-2) | 12/24 porte collegate + modulo di assegnazione completo; 9 chiuse per costruzione (tenant-materialization, observability, tenant-blueprints — solo PLATFORM_ADMIN le raggiunge oggi) rinviate a R-9/R-5; 3 (content-blueprint-links) fuori perimetro per ScopeFilter condiviso — dettagli in esiti/R-0.md. ⚠ SOSPESA non per un rosso: `verify_gate.py run` sull'ultimo commit (f8d7e560) sta girando in background sul gemello (avviato 01:03, PID indipendente dalla sessione — sopravvive alla sua chiusura) e non ha ancora scritto il verdetto quando il guardiano ha imposto lo stop (74.8%). La sessione che riprende legge PRIMA `.zp/verify-verdict.json`: se `head=f8d7e560` e verdetto GREEN, chiude R-0 senza altro lavoro; se RED, corregge quel solo difetto. CI remota: idem, verificare `gh run list` per f8d7e560 |
