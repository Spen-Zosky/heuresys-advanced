...
| 28 | 2026-09-19 | close-propagate SIGPIPE in modalità --delta | APERTA | `align-claude-ecosystem.sh` riga ~601: pipeline `find\|head` sotto `pipefail` ancora priva di `\|\| true` | aggirato con --full, causa radice non corretta nel codice |
...
| 30 | 2026-09-19 | bundle produzione fermo al 14/9, 3 ruoli senza permessi effettivi in prod | APERTA | `STATO.md` R-2 (S1106) e voce D11 (S1108, 2026-09-25): nessuna conferma di deploy successivo | stato live del bundle NON MISURATO oggi (richiede SSH VM, fuori scope); risulta ancora aperto nella documentazione più recente |
