...
| E | Se falliti = 0 → passaggio della suite in CI (criterio `#211` F4) | io | il workflow esegue la suite e la CI è verde | ⛔ **NON RAGGIUNTO** — dopo la correzione i falliti sono **6** (erano 42): il criterio di `#211` chiede zero, quindi la suite non entra ancora in CI. Non è una voce saltata: è una voce il cui esito è ROSSO, con la causa dominante rimossa e i sei residui nominati uno per uno |
...
| G | *(aggiunta su istruzione di Enzo: «risolvi il disallineamento del token e rilancia»)* Rimuovere la causa dominante e rimisurare | io | la famiglia di falliti sparisce, misurata da una corsa integrale nuova | ✅ **FATTO** — non era il token, header e cookie **combaciavano**: era l'**origine** rifiutata. `ADMIN_ORIGIN` accetta ora un elenco (`955d1853`), con 10 prove falsificabili e un preflight che lo misura interrogando l'API. Corsa rifatta: **da 42 falliti a 6**, i 403 sulle scritture spariti per intero |
