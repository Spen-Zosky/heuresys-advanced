...
- [x] **F10 `#76`** — **fatto =** verifica del piano scritta: cluster ancora validi / superati / già fatti, con numeri — FATTA 2026-09-12 · agente Sonnet delegato (sola lettura, 345k token, 60 tool call): 216/216 verificati → **49 GIÀ-FATTO · 10 SUPERATI (I12) · 153 VALIDI = 974,2 h · 4 NON-MISURABILI** in `.programmi/76-verifica-piano-zero-pendenze.md`; `#76` HOLD → **ACTIVE** (la sessione preliminare che Enzo pretendeva è fatta), prossimo passo F2 = riportare i 59 verdetti nel piano
...
- [ ] **F12 `#205` F2** — **fatto =** la fase «indirizzi» legge la sitemap/cerca; corsa su `positions` con proposte decise e applicate — **PARZIALE** 2026-09-12: lo strumento c'è (passo ⓪ «mappa del sito» nell'API, candidati reali al modello, scarto per costruzione nel gateway; unit 17/17, typecheck+lint verdi); la **corsa NON è stata eseguita** (catena della ricerca + tunnel degradato). La fase resta aperta nel piano #205
- [ ] **F13 `#205` F3** — **fatto =** frase riconoscibile → zero riscontri nello strato di forma — **NON FATTA**: dipende da F12 (serve una proposta approvata del cliente A, e la corsa non è stata eseguita)
- [ ] **F14 `#198` T9b** — **fatto =** azienda non bancaria costruita dal modello generato, prove 11/11 sul gemello poi in produzione — **NON FATTA**: il modello `MGMT_CONSULTING_SMALL` resta vuoto finché #205 F2 non produce; costruire oggi = un'altra banca (stessa ragione di S1096)
- [ ] **F15 `#159` F2** — **fatto =** componente pubblicato in `@heuresys/ui`, la console dev lo consuma, secondo consumatore montato — **NON FATTA (capienza, misurata)**: guardiano alle 22:40 `mancano 233.101 token · 5h: mancano 45 punti`; la fase ne chiede ~250k più la chiusura (~100k). Il repo `ux-design-shared` è LIBERO (misurato al boot: la sessione registrata era un pid riusato): la prossima sessione può aprirla per prima
...
| F12 | fonti ammesse nel mandato (S1096) · lettura sitemap via `m.leggi` (stesse guardie), ordine per attinenza, `candidati` al gateway · deploy normale · io · tetti: 12 mappe/corsa, 120 candidati; senza mappa si chiede come prima | l'ordine per parola intera era cieco alla morfologia: corretto con la radice a 6 caratteri, colto dal test |
| F10 | piano leggibile, DB in sola lettura · un agente Sonnet (delega per le parti: 216 letture, io decido sul riepilogo) · file in `.programmi/` + register · io · agente senza scrittura sul repo (un solo file), niente SSH, niente verify_gate | quasi un quarto del piano era già chiuso da lavoro mai riconciliato; W6 non si è sbloccato da solo |
...
- **Il `claude` del gemello e della VM ha la sessione OAuth scaduta** (S1096, ancora vero per quanto si sa: non ri-misurato oggi). Senza, la corsa di `#205` F2 gira solo col modello su Windows. Vuoi ri-loggare le due macchine?
- **`#240`**: i due worktree gov portano solo contenuto superato — serve il tuo sì alla rimozione (è nel register come WAIT-INPUT).
- **`#250`**: resta tua, come in S1096.
- **Il tunnel di casa verso la VM era degradato stasera** (7,8 s per una `count` su 164 righe, VM a load 0.05): non è un difetto del progetto, ma ha impedito l'E2E locale e ha fatto morire un `psql` in background. Se si ripete, la memoria `degraded_tunnel` dice di ricrearlo.
- **`next.config.js` in dev proxyava a se stesso** (corretto oggi): chi ha lanciato E2E locali dopo #219 (2026-09-05) e ha visto 12 `auth.setup` rossi con «Impossibile contattare il server» stava vedendo questo, non il prodotto.
- **4 cluster NON-MISURABILI** della verifica di `#76` (Z-021, Z-090 vogliono SSH; Z-050, Z-167 una scrittura): da guardare nella sessione che riprende `#76`.
