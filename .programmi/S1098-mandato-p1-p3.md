...
|---|---|---|---|---|---|
| 1 | `#149` | F4 — la consegna ingerita oggi (P4) trattata come non verificata | io | l'analisi avversariale di `#206` è registrata come esito di F4 | — |
| 2 | `#79` | F3 — il lavoro che popola tabelle (P4) passa dal cancello | io | `check_exposure.py` → 0 lacune dopo `#206` | — |
| 3 | `#76` | F2 — riportare i verdetti nel piano zero-pendenze | io | `zp_state.py piano` ri-conta; i 59 verdetti sono nel piano | — |
| 4 | `#214` | F6 — il prossimo perimetro neutro | io | una riga in `agent-perimetri.json` + `check_concetti_agente.py` verde | — |
| 5 | `#205` | F2 — la corsa su `positions` con la fase «indirizzi» | io | `fonti` nell'esito della corsa, o la ragione misurata per cui non produce | — |
| 6 | `#159` | F2 — il ponte in `ux-design-shared` | io | ⛔ fuori confine salvo budget: ~250k | — |
