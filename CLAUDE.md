...
| **Quanto contesto ho consumato** — misurato, mai stimato | `python docs/kb/tools/context_meter.py` · `--budget N` (exit 2 = non ci sta) · `--stop-at 0.75` (exit 3 = soglia raggiunta) · `--selftest`. Legge i token che l'**API** ha riportato nel transcript JSONL della sessione: `input + cache_read + cache_creation`. Il boot lo stampa già in cima alla dashboard, in **entrambe** le modalità |
...
- **Il contesto si misura, non si stima** (Enzo, 2026-08-13 — vale in **ogni** sessione, canonical e lab). Nessuna frase sul contesto residuo — «si stringe», «siamo al limite», «meglio chiudere» — senza l'output di `context_meter.py` accanto. Il numero è un **pavimento** (il turno in corso non è ancora nel transcript), quindi a ridosso di una soglia la si considera raggiunta, non si tira. Se il transcript non si trova si dichiara **NON MISURABILE** e non si torna a intuire. Misurare **non** è decidere: la chiusura resta di Enzo, e se lui fissa una soglia d'arresto la si rende meccanica con `--stop-at`, non la si affida al ricordo.
