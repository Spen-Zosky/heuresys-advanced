...
| 🔒 **Regole fatte rispettare dal motore, non ricordate** — divieti git, segreti (lettura e pre-commit), cancellazioni, scritture sul DB vivo (`psql :5433`, `db:reset`, `db:migrate` da Windows), Python UTF-8, guardiano in riga di stato, file GENERATO, tool MCP di invio | plugin `plugins/enzo-guard/` (README con la tabella degli hook, come spegnerne uno da `/config`, come tornare indietro) | in ogni sessione su Windows via `~/.claude/skills/enzo-guard`; **non** su Mac/VM/linux-pc, Cowork, Codex, cron — lì valgono le righe di questo file |
