...
| D | Brownfield Wave 1 execution | ✅ | Framework completo + pipeline end-to-end VERDE su debug-scale (registry 93+1164+94+1177 row, modulo executor, 5 endpoint, staging 17 tables, legacy data 356MB via script, validation set-based, LOOKUP_FK resolver con pre-loaded cache, topological sort 17 target, required-col fallback). Integration test gated `BROWNFIELD_RUN_REAL_WAVE1=1 WAVE1_DEBUG_LIMIT=5` passa con `totalUpserted > 0` (sys_skills=10, sys_user_certifications=1, sys_blueprint_process_registry=23 popolati su Tappa D run cumulative), lineage scritti, `state=COMPLETE`. **Known issue performance**: full execution (47k row, no DEBUG_LIMIT) e mid-scale (200/target) crashano Node Windows con OOM kill — bottleneck CPU/memoria in JS (FK cache refresh + buildTargetRow loop su molti row). Refactor performance separato (worker_threads o chunked transaction o COPY-based bulk) richiesto per scale completa. 218/219 vitest verde (1 gated) |
