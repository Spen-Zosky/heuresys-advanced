...
export const capabilityMaturityRoutes: FastifyPluginAsyncZod = async (app) => {
  app.get(
    "/maturity",
    { config: { orgGate: "aggregate", tenantGate: "service" }, preHandler: [requirePermission("capability:read")], schema: { response: { 200: CapabilityMaturityListResponseSchema } } },
    async (req) => capabilityMaturityService.maturity(actor(req)),
  );

  app.get(
    "/maturity/org-units/:orgUnitId",
    { config: { orgGate: "aggregate", tenantGate: "service" }, preHandler: [requirePermission("capability:read")], schema: { params: CapabilityMaturityOrgUnitParamSchema, response: { 200: CapabilityMaturityScoreSchema } } },
