...
beforeAll(async () => {
  ...
  const ids = await pool.query<{ user_id: string; user_email: string }>(
    ...
    [["federica.marchetti@rtl-bank.org", "paolo.caputo@rtl-bank.org", "tommaso.fiore@rtl-bank.org", "platform-test-admin@collaudo.invalid"]],
  );
  ...
  adminId = byEmail.get("platform-test-admin@collaudo.invalid")!;
