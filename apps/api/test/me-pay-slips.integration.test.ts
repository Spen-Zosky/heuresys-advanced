...
let noSlips: string;      // platform-test-admin@collaudo.invalid — platform account, never has slips
...
describe("/v1/me/pay-slips — F4 cedolini", () => {
  beforeAll(async () => {
    ...
    expect(Number(rows[0]?.n ?? -1), "platform-test-admin@collaudo.invalid deve essere senza cedolini").toBe(0);
    noSlips = await cookie(suite, "platform-test-admin@collaudo.invalid");
  });
