 ...
 * D-14 FASE 1 — POST /v1/tenants/provision. Verifies the whole transactional
 * chain (tenant + first TENANT_ADMIN identity/credential/role + MFA policy),
 * that the provisioned admin can immediately authenticate, admin-gating, and
 * atomic rollback on a duplicate tenantCode.
 */
...
describe("D-14 tenant provisioning /v1/tenants/provision", () => {
  ...
  it("PLATFORM_ADMIN provisions a full tenant in one transactional call", async () => {
    ...
    const out = r.json() as {
      ...
      admin: { userId: string; email: string; mustRotatePassword: boolean };
    };
    ...
    expect(grant.rows.map((x) => x.auth_role_code)).toContain("TENANT_ADMIN");
