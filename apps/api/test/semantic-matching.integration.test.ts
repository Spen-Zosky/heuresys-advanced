...
let suite: TestApp;
let admin: S;        // platform-test-admin@collaudo.invalid (PLATFORM_ADMIN) — sees all
...
beforeAll(async () => {
  ...
  const ids = await pool.query<{ user_id: string; user_tenant_id: string; user_email: string }>(
    `SELECT user_id, user_tenant_id, user_email FROM sys.sys_users
     WHERE user_email IN ('paolo.caputo@rtl-bank.org','antonio.parisi@rtl-bank.org','platform-test-admin@collaudo.invalid')`);
  ...
  const heuTenant = by("platform-test-admin@collaudo.invalid").user_tenant_id;
