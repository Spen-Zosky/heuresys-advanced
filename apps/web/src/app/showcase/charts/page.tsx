...
function Heatmap() {
  ...
  return (
    <div className="overflow-x-auto">
      <svg width={(HEATMAP.cols.length + 1) * cellW + 20} height={(HEATMAP.rows.length + 1) * cellH + 10} role="img" aria-label="Skills coverage by department">
        <g transform={`translate(${cellW},${cellH - 8})`}>
          {HEATMAP.cols.map((c, i) => (
            <text key={c} x={i * cellW + cellW / 2} y={0} textAnchor="middle" fontSize="11" fill="#475569">{c}</text>
          ))}
        </g>
        {HEATMAP.rows.map((r, ri) => (
          <g key={r} transform={`translate(0,${(ri + 1) * cellH})`}>
            <text x={cellW - 8} y={cellH / 2 + 4} textAnchor="end" fontSize="11" fill="#475569">{r}</text>
            {HEATMAP.cols.map((_, ci) => {
              const v = HEATMAP.values[ri]![ci]!;
              const opacity = 0.15 + (v / 100) * 0.85;
              return (
                <g key={ci} transform={`translate(${(ci + 1) * cellW},0)`}>
                  <rect x={2} y={2} width={cellW - 4} height={cellH - 4} fill="#2563EB" opacity={opacity} rx={3} />
                  <text x={cellW / 2} y={cellH / 2 + 4} textAnchor="middle" fontSize="11" fill={v > 60 ? "#FFFFFF" : "#0F1828"}>{v}%</text>
                </g>
              );
            })}
          </g>
...
function LineChart() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} role="img" aria-label="Learning completions trend YTD" className="w-full">
      <line x1={pad} y1={h - pad} x2={w - pad} y2={h - pad} stroke="#E2E6EE" />
      <line x1={pad} y1={pad} x2={pad} y2={h - pad} stroke="#E2E6EE" />
      <path d={path} fill="none" stroke="#2563EB" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      <path d={`${path} L${points[points.length - 1]![0]},${h - pad} L${pad},${h - pad} Z`} fill="#2563EB" opacity="0.08" />
      {points.map((p, i) => <circle key={i} cx={p[0]} cy={p[1]} r="3" fill="#2563EB" />)}
      {labels.map((l, i) => (
        <text key={l} x={pad + i * step} y={h - pad + 14} textAnchor="middle" fontSize="10" fill="#64748B">{l}</text>
      ))}
    </svg>
  );
}
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} role="img" aria-label="Position fill rate by department" className="w-full">
      {data.map((d, i) => {
        ...
        return (
          <g key={d.dept}>
            <text x={pad - 8} y={y + barH / 2 + 4} textAnchor="end" fontSize="11" fill="#475569">{d.dept}</text>
            <rect x={pad} y={y} width={w - pad - 30} height={barH} fill="#E2E6EE" rx={3} />
            <rect x={pad} y={y} width={barW} height={barH} fill="#2563EB" rx={3} />
            <text x={pad + barW + 6} y={y + barH / 2 + 4} fontSize="11" fill="#0F1828">{d.fill}%</text>
          </g>
        );
      })}
    </svg>
...
function Donut() {
  const segments = [
    { label: "Critical (>2 gaps)", value: 47, color: "#DC2626" },
    { label: "Watch (1–2 gaps)", value: 132, color: "#F59E0B" },
    { label: "Healthy (0 gaps)", value: 163, color: "#16A34A" },
  ];
  ...
  return (
    <svg viewBox="0 0 240 180" role="img" aria-label="Skill gap distribution" className="w-full">
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" fontSize="22" fontWeight="600" fill="#0F1828">{total}</text>
      <text x={cx} y={cy + 12} textAnchor="middle" fontSize="10" fill="#64748B">positions</text>
      <g transform="translate(170,32)" fontSize="11">
        {segments.map((s, i) => (
          <g key={s.label} transform={`translate(0,${i * 22})`}>
            ...
            <text x="16" y="9" fill="#0F1828">{s.label}</text>
            <text x="16" y="22" fill="#64748B" fontSize="10">{s.value} positions</text>
          </g>
        ))}
      </g>
    </svg>
