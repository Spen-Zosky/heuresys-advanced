...
/**
 * DataTablePanel — canonical list-page composition (brand-component-contract.md:
 * "Data table / entity list" → DataTableWithCrossHair + StatusPill, with
 * PageHeader, EmptyState, ErrorState). Presentational: the page owns the query
 * and passes resolved rows + column descriptors. Built purely from @heuresys/ui
 * primitives; lives in apps/web as tenant-domain composition.
 *
...
export interface DataTablePanelProps<T> {
  /** testid for the page <main> (e.g. "users-page"). */
  pageTestId: string;
  /** testid attached to the PageHeader (e.g. "users-title"). */
  titleTestId?: string;
  /** testid for the count badge (e.g. "users-count"). */
  countTestId?: string;
  title: string;
  description?: string;
