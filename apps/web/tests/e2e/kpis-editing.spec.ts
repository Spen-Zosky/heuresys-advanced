...
test.describe("catalogo KPI — definizione e correzione", () => {
  ...
  test("definisce un indicatore, lo ritrova cercandolo, lo corregge e lo rimuove", async ({ page, request }) => {
    ...
    try {
      ...
      await expect(page.getByTestId("kpis-page")).toBeVisible({ timeout: 45_000 });
      ...
      await page.getByTestId("kpi-create-name").fill(`Indicatore di collaudo ${marca}`);
      await page.getByTestId("kpi-create-unit").fill("%");
      await page.getByTestId("kpi-create-polarity").selectOption("LOWER_IS_BETTER");
      const [post] = await Promise.all([
        page.waitForResponse((r) => r.url().includes("/v1/kpi-definitions") && r.request().method() === "POST"),
        page.getByTestId("kpi-create-submit").click(),
      ]);
      expect(post.status(), "definizione indicatore non accettata").toBe(201);
      const creato = (await post.json()) as { kpiDefinitionId: string; polarity: string; unit: string | null };
      kpiId = creato.kpiDefinitionId;
      // i valori scelti nei menù sono arrivati davvero, non i predefiniti
      expect(creato.polarity, "la polarità scelta non è stata salvata").toBe("LOWER_IS_BETTER");
      ...
      await expect(pulsante, "la ricerca non ha trovato l'indicatore appena definito").toBeVisible({
        timeout: 30_000,
      });

      // --- correzione dall'interfaccia ---
      await pulsante.click();
      await expect(page.getByTestId("kpi-editor")).toBeVisible({ timeout: 30_000 });
      await expect(page.getByTestId("kpi-edit-code")).toBeDisabled();
      await expect(page.getByTestId("kpi-edit-polarity")).toHaveValue("LOWER_IS_BETTER");
      ...
      const [patch] = await Promise.all([
        page.waitForResponse(
          (r) => r.url().includes(`/v1/kpi-definitions/${kpiId}`) && r.request().method() === "PATCH",
        ),
      ...
      const dopo = await request.get(`${API_BASE}/v1/kpi-definitions/${kpiId}`, { headers: { cookie } });
      const letto = (await dopo.json()) as { name: string; unit: string | null };
      expect(letto.name, "la correzione del nome non è arrivata al database").toBe(nuovoNome);
      expect(letto.unit, "la correzione dell'unità non è arrivata al database").toBe("giorni");
    } finally {
      // i KPI hanno una DELETE sull'API: la prova ripulisce e lo VERIFICA
      if (kpiId) {
        const del = await request.delete(`${API_BASE}/v1/kpi-definitions/${kpiId}`, {
