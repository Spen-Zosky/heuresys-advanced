...
test.describe("catalogo formativo — inserimento e correzione", () => {
  ...
  test("inserisce un modulo, lo ritrova cercandolo, lo corregge e lo rimuove", async ({ page, request }) => {
    ...
    try {
      ...
      await page.getByTestId("learning-create-title").fill(`Corso di collaudo ${marca}`);
      await page.getByTestId("learning-create-duration").fill("90");
      await page.getByTestId("learning-create-kind").selectOption("WORKSHOP");
      await page.getByTestId("learning-create-delivery").selectOption("INSTRUCTOR_LED");
      const [post] = await Promise.all([
        page.waitForResponse((r) => r.url().includes("/v1/learning-modules") && r.request().method() === "POST"),
      ...
      const creato = (await post.json()) as {
        learningModuleId: string; kind: string; delivery: string; durationMinutes: number | null;
      };
      moduleId = creato.learningModuleId;
      // i valori scelti sono arrivati davvero, non i predefiniti…
      expect(creato.kind, "il tipo scelto non è stato salvato").toBe("WORKSHOP");
      expect(creato.delivery, "l'erogazione scelta non è stata salvata").toBe("INSTRUCTOR_LED");
      // …e la durata è un NUMERO, non la stringa del campo
      expect(creato.durationMinutes, "la durata non è arrivata come numero").toBe(90);
      ...
      await expect(pulsante, "la ricerca non ha trovato il modulo appena inserito").toBeVisible({
        timeout: 30_000,
      });

      // --- correzione dall'interfaccia ---
      await pulsante.click();
      await expect(page.getByTestId("learning-editor")).toBeVisible({ timeout: 30_000 });
      await expect(page.getByTestId("learning-edit-code")).toBeDisabled();
      await expect(page.getByTestId("learning-edit-kind")).toHaveValue("WORKSHOP");
      await expect(page.getByTestId("learning-edit-duration")).toHaveValue("90");
      ...
      const [patch] = await Promise.all([
        page.waitForResponse(
          (r) => r.url().includes(`/v1/learning-modules/${moduleId}`) && r.request().method() === "PATCH",
        ),
        page.getByTestId("learning-edit-save").click(),
      ]);
      expect(patch.status(), "la correzione non è stata accettata dall'API").toBe(200);

      // PROVA VERA: titolo e durata nuovi sono nel database
      const dopo = await request.get(`${API_BASE}/v1/learning-modules/${moduleId}`, { headers: { cookie } });
      const letto = (await dopo.json()) as { title: string; durationMinutes: number | null };
      expect(letto.title, "la correzione del titolo non è arrivata al database").toBe(nuovoTitolo);
      expect(letto.durationMinutes, "la correzione della durata non è arrivata al database").toBe(120);
    } finally {
      if (moduleId) {
        const del = await request.delete(`${API_BASE}/v1/learning-modules/${moduleId}`, {
          headers: await deleteHeaders(page),
        });
        expect(del.status(), `pulizia non riuscita (HTTP ${del.status()})`).toBe(204);
      }
    }
  });
});
