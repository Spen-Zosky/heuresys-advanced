...
| **59** | **Bisect methodology contamination + Next 15 RSC bundle threshold (architectural)**. Two-layer bias: (a) bisect via `export removal` in src/index.ts contamina apps/web typecheck (consumer imports collettivi blocked), (b) stub replacement con `const X = null` cambia module structure (was pure re-exports, became mixed) → dist output diverso, webpack chunking differs, fail behavior changes, bisect convergence fallisce; (c) link: vs versioned hanno fail modes diversi (link: webpack parse error on `useEffect`, versioned `d.createContext undefined`) — bisect via link: ABANDONED. (Architectural) Next.js 15.5.18 ha bundle complexity threshold per RSC server bundle: quando `@heuresys/ui` exports >50 components, page-data collection di `/showcase/*` routes trippa `d.createContext is not a function` (React minified a `d` undefined in chunk evaluation context). Threshold reproducible: iter 6 (3 of 7 obs widgets) PASS, iter 7 (full 7) FAIL. Individual components hanno minimal-imports — issue è emergent chunk topology + bundle size, NOT singolo culprit. **Manifestation X18.4**: 12 bisect iterations CLI, NO single-component culprit identifiable, REAL cause = architecturale Next 15 RSC non risolvibile in X18 scope. **S935 PHASE C REFRAME (2026-05-26)**: l'iter 12 empirical stack-trace (`qa_artifacts/x18_4_bisect_iter_12.txt`) mostra che la fail è `TypeError: d.createContext is not a function`, NOT un bundle-size error. La narrazione "RSC bundle threshold emergent" è una **narrative-bias** (CW-B58 lesson). Vera root cause: **React peer-dep mismatch** o `'use client'` missing su Radix-UI components di `@heuresys/ui`, oppure CJS/ESM interop drift (tsup dual output). Vedi `docs/cw-b59-true-root-cause-2026-05-26.md` per analysis + 3 path strategy (G React pinning quick-win, A revised message-grep bisect, F split @heuresys/ui fallback). | CLI X18.4 halt_bisect_inconclusive §2-§5 + 12 iter empirical evidence + S935 phase C reframe | **RESOLVED (CLI 2026-05-27, app-side ssr:false — Path F-alternative chosen over package split)** — root cause confermata empiricamente = CJS/ESM interop: il single-barrel `@heuresys/ui` valuta eager le lib class-based pesanti (three/@react-three, echarts, d3, recharts) al module-load; la SSR/static page-data collection le valuta → `Class extends value undefined`. Fix: `apps/web/src/app/showcase/_ui-client.tsx` ri-esporta i componenti showcase via `next/dynamic({ ssr:false })` → il barrel non è mai valutato in SSR build (rendering client-side, accettabile per demo brand). Rimosso il vecchio workaround `force-dynamic` (incompatibile con `output:export`). Verificato: `pnpm --filter @heuresys/web build` exit 0 + `build:static` exit 0 + CI **build-web GREEN + showcase deploy GREEN** (primo verde dal 2026-05-20) su HEAD `7f6e174`. Path F (split npm package) NON necessario. Closes DEFER-F. | REPORT 022 §6-RESUMED-X18.5; halt notify `2026-05-24T18-32-00Z__022__halt_bisect_inconclusive.md`; PROMPT 022.5; `docs/cw-b59-true-root-cause-2026-05-26.md`; **commit `7f6e174` + `apps/web/src/app/showcase/_ui-client.tsx`** |

### Bias surfaced S937 housekeeping (post-S936 follow-up — empirical SSH automation gap)

| # | Nome breve | Originator | Status | Riferimento |
|---:|---|---|---|---|
| **62** | **MCP-Windows SSH interactive automation gap** — `Start-Process powershell -WindowStyle Normal` lanciato da Windows-MCP PowerShell apre process child ma la finestra GUI o resta minimizzata in taskbar (handle visible ma non foreground) o si chiude in pochi secondi senza che l'utente abbia tempo di interagire con il prompt passphrase. 3 varianti testate S937 CK-1: (a) `Start-Process -Wait` torna troppo presto, (b) `cmd /K` con title custom apre + diventa focused per ~1s poi diventa unfocused, (c) helper .ps1 con `Read-Host` finale chiude comunque prima. Causa probabile: MCP sessione SessionId=1 stessa di console Enzo, ma le finestre spawn-ate via Start-Process non ereditano interactive token correttamente. R23/iii eccezione canonica per passphrase entry: passphrase non passabile da stdin via MCP (sarebbe R11 violation se in plaintext, e named pipe ssh-agent richiede prompt sincrono). Mitigazione: documentare istruzioni esecutive in HANDOFF.md + chiedere a Enzo manual launch da shell aperta a mano. | Cowork S937 CK-1 empirical (3 tentativi) | **RESOLVED (S937 CLI continuation 2026-05-27)** — al boot della sessione CLI la chiave `oci_recovery_ed25519` era già caricata in ssh-agent + tunnel 5433 up; registrazione runner OCI VM completata in autonomia (token via `gh api`, no UI). Mitigazione originale: helper script `C:\Users\enzospenuso\Claude Desktop\scripts\s937-ck1-load-ssh-key.ps1` (R17 workspace-level) per Enzo manual launch. Pattern memo per futuri SSH passphrase task: NON tentare automation MCP, scrivere helper + delegare a Enzo subito con istruzioni esplicite. Path alternativo: convertire chiave OCI a no-passphrase (security trade-off, NO senza ADR) OR creare service-account key dedicata CI no-passphrase. | HANDOFF.md §S937 CK-1; `scripts/s937-ck1-load-ssh-key.ps1`; qa_artifacts/s936_outcome_summary.md §5 finding #1 |
| **63** | **CI workflow "shipped" ma mai eseguito → drift di config latente che emerge solo al primo run reale**. I 6 workflow GitHub Actions self-hosted erano stati scritti S935-F e considerati deliverable "shipped", ma mai eseguiti (registrazione runner deferita). Al primo run reale (S937 CLI continuation) sono emersi **8 bug di config distinti** mai catturati: (1) systemd EnvironmentFile unescape `\n`→`n` su JWT PEM → FAST_JWT_INVALID_KEY (tutte 42 suite down); (2) `POSTGRES_DB` vs `POSTGRES_DATABASE`; (3) `NEXT_PUBLIC_API_BASE_URL` build-time mancante; (4) `ADMIN_ORIGIN` CORS porta sbagliata; (5) seed-check `email` vs `user_email`; (6) `--grep "@smoke"` su tag inesistente → "No tests found"; (7) collisione porta :3000 = Grafana sul runner host → web E2E; (8) server backgrounded killati tra step + `next start` script con `-p 3000` hardcoded. META-lezione: un'automazione "shippata" ma non eseguita NON è validata — la prima esecuzione fa parte del deliverable, non è una formalità. Affine a CW-B52 (spec staleness): codice/config scritto senza run reale accumula assunzioni non verificate. | Cowork→CLI S937 CK-6 empirical (8 iterazioni fix) | **MITIGATED**: tutti e 8 fixati, 6/6 workflow verdi su `01340ae`; doc `docs/ci/self-hosted-runners-setup.md` §3-4 aggiornato col contratto verificato + warning escaping systemd. Pattern memo: per ogni CI/automation deliverable, eseguire ALMENO un run reale end-to-end prima di marcarlo "shipped". | HANDOFF.md §2026-05-27; commit `9fff8e9`→`01340ae`; `docs/ci/self-hosted-runners-setup.md` §4 |

### Bias surfaced post-X19 Brownfield Wave 1 re-run (REPORT 023 §6 — empirical HALT-X19-01)

| # | Nome breve | Originator | Status | Riferimento |
|---:|---|---|---|---|
| **60** | **Engine silent-filter on approved+validated IMPORT mappings (upsert yields 0 rows, no WARNING log) + Wave-1 scope gap (IMPORT targets without staging source)**. Two-part residual surfaced when the post-CW-B49 engine (HEAD `b01c331`, commit `7ea09f0`) was run end-to-end on Wave 1 for the first time (run `6f531559`, COMPLETED 47min, 34509 upserted, 0 failed). (A) 3 targets `sys_skill_categories` (32 staging), `sys_activity_classification_mappings` (14), `sys_process_kpi_templates` (81) were AUTO_APPROVED + 0 validation failures, yet `executeUpsert` produced **0 target rows with zero WARNING/ERROR logs** → silent row-drop (NK/required/FK filter or SKIPPED_UNSUPPORTED_TRANSFORM, beyond CW-B49 conflict-inference). (B) 3 targets `sys_blueprint_overrides`, `sys_position_learning_requirements`, `sys_position_skill_requirements` have NO `staging.wave1_*` source → `stagingTableFor`→null → silent skip (no Wave-1 source export; derived/computed or Wave 2 scope). META: PROMPT 023 acceptance `≥75/134 populated` was unreachable — only 19 distinct IMPORT targets exist in Wave 1 (CW-B52 spec staleness; reality 59/134 pre+post, 13/19 IMPORT targets populated). | CLI X19 §6 empirical (post-patch full run) | **(A) MITIGATED via CW-B61** (S934 2026-05-26 — observability fix shipped); **(B) MITIGATED via ADR-0020** (S935 2026-05-26 — 3 application-level targets reclassified IMPORT→REFERENCE_ONLY; migration 000044 idempotent). | REPORT 023 §6 + halt notify `__023__halt_engine_residual_6_targets.md`; run `6f531559`; CW-B61 entry; ADR-0020 + migration 000044 |
| **61** | **Silent-skip observability gap in `executeUpsertSqlSidePerMapping`: main INSERT rowCount=0 returned without log nor audit row**. Root cause for the (A) part of CW-B60. Code path `upsert-sql.ts:763-765` returned `{ upsertedRows:0, skipped:false }` when `pool.query(insertSql).rowCount === 0`. With `skipped:false`, the engine.ts:840 logger.error branch `if (sqlResult.skipped && sqlResult.skipReason)` was skipped → no pino emit. CW-B17 audit (`WHERE_SKIP_FILTER_EXCLUDED_V1`) only covers per-row exclusions from `skipFilters`, NOT the main-INSERT rowCount=0 case. Triggered for the 3 CW-B60-A targets because all 3 lack a `_tenant_id` NK (no COALESCE-sentinel UQ pattern, CW-B49 inapplicable); their `column_mappings` cover only NK cols → `setClauses.length===0` → `ON CONFLICT DO NOTHING` → rowCount=0 on duplicate / re-run inputs. | Cowork S934 forensic (post-CODE-1 pino logger shipped) | **MITIGATED**: probe `SELECT count` + `logger.warn` structured + audit row `SILENT_UPSERT_ZERO_ROWS_V1` (status='SKIPPED') emitted at `upsert-sql.ts:763` before return. Result shape unchanged for back-compat. 3 unit tests (`apps/api/test/upsert-sql-cw-b60-a-silent-skip.test.ts`) verify (T1) silent skip emits audit, (T2) happy path stays quiet, (T3) DRY_RUN no audit. | `apps/api/src/modules/brownfield-wave-executor/upsert-sql.ts:763-875` + `audit-rule-codes.ts:49` + `apps/api/test/upsert-sql-cw-b60-a-silent-skip.test.ts` |

---

## §3 — Next available

**Next available**: `CW-B64`
...
- **Total catalogati**: 62 actively catalogued (CW-B17 → CW-B63, **CW-B57 WITHDRAWN 2026-05-24 per misdiagnosis**). Effective valid bias = 62 (B17→B56 + B58→B63).
- **Mitigated/Resolved**: 45 (+CW-B49 X10 engine patch, +CW-B51 X11 inline SKIPPED, +CW-B52 X12 inline audit-refresh, +CW-B53 X13 inline functional-reading adoption, +CW-B54 X14 inline verdict-separation, +CW-B55 C18.1 PROMPT amendment + pre-flight grep mandatory, +CW-B56 C18.2 PROMPT amendment + npm pre-flight extended (org/2FA/GAT), +CW-B58 C18.3 PROMPT amendment + outExtension fix + pre-prescription bundle inspection mandatory (reinforced cross-batch X18.4/X18.5), +CW-B59 C18.5 pragmatic Path B+C workaround + bisect methodology canonical + Next 15 RSC bundle threshold deferral, +CW-B60-A via CW-B61 S934 silent-skip observability fix + 3 unit tests, +CW-B60-B via ADR-0020 + migration 000044 S935 reclassify 3 application-level targets, +CW-B61 S934 inline, +CW-B62 S937 RESOLVED via CLI continuation 2026-05-27 (SSH agent loaded + autonomous runner registration), +CW-B63 S937 CI first-run config drift MITIGATED 8/8 fixes + 6/6 workflows green)
...
- **Deferred proper fix**: 0 (CW-B59 RESOLVED 2026-05-27 via app-side `ssr:false` client boundary — DEFER-F closed; Path F package split not needed)
