...
| **57** | tsup `external` minimal default + subpath exports source-direct = **dual-package hazard certified**. tsup config baseline `external: ["react", "react-dom"]` bundles tutte le altre runtime libs (Radix UI ~25 entries, framer-motion, tanstack, react-three, reactflow, react-hook-form, ecc.) inline nel dist/index.{cjs,mjs}. Quando consumer (apps/web/apps/showcase) carica via main entry → riceve la copia bundled; quando carica via subpath source-direct (`@heuresys/ui/brand/candidates` → `src/components/brand/candidates/index.ts` con `import { Dialog } from "@radix-ui/react-dialog"`) → pnpm resolve via `node_modules/.pnpm/@radix-ui+react-dialog@x` (transitive di @heuresys/ui dependencies declaration). Due istanze React.createContext distinte → `React.useContext()` ritorna `undefined` per provider della copia diversa → crash `TypeError: Class extends value undefined is not a constructor or null` al build collect-configuration di Next.js. Type-only checks PASS, runtime FAIL. Manifestation MVP-3 X18 commit `ef46668` (0.1.0): dist/index.cjs 388 KB con Radix bundled, apps/web build su /showcase route fallita. | CLI X18 halt_dual_package_hazard §1-§5 + 6 verified-by test | **mitigated (Cowork C18.2 EVIDENCE)** — PROMPT 022.2 amendment §2 A.2 tsup.config.ts `external` extended a regex `/^@radix-ui\//`, `/^@tanstack\//`, `/^@react-three\//`, `/^d3-/`, `/^embla-carousel/` + lista esplicita context-bearing libs (framer-motion, three, reactflow, @xyflow/react, react-hook-form, @hookform/resolvers, zod, cmdk, sonner, recharts, plotly.js, chart.js, react-chartjs-2, vaul, date-fns, dayjs, react-day-picker, next/*, ecc.). Sanity check post-build: dist/index.cjs < 100 KB target (4x reduction vs 388 KB broken baseline). Version bump 0.1.0 → 0.1.1 + `npm deprecate @heuresys/ui@0.1.0` post-republish (best practice no-unpublish). Pattern memo C19 task: documentare "se exports map contiene subpath source-direct, tsup external DEVE includere TUTTE le runtime libs context-bearing — altrimenti dual-package hazard certificato". | REPORT 022 §1-RESUMED + §6-RESUMED; halt notify `2026-05-24T16-32-00Z__022__halt_dual_package_hazard.md`; PROMPT 022.2 §2 |

---

## §3 — Next available

**Next available**: `CW-B58`
...
- **Total catalogati**: 57 bias (CW-B17 → CW-B57)
- **Mitigated**: 38 (+CW-B49 X10 engine patch, +CW-B51 X11 inline SKIPPED, +CW-B52 X12 inline audit-refresh, +CW-B53 X13 inline functional-reading adoption, +CW-B54 X14 inline verdict-separation, +CW-B55 C18.1 PROMPT amendment + pre-flight grep mandatory, +CW-B56 C18.2 PROMPT amendment + npm pre-flight extended (org/2FA/GAT), +CW-B57 C18.2 PROMPT amendment + tsup external aggressive regex)
