...
VALUES
  ...
  ('sys_generated_record_origins', 'D', 'EXCLUDE', NULL,
   '[S1066] Tenant Builder P3/T1 (mig 000319) — registro dell''origine delle righe GENERATE da una versione di fascicolo. Dato prodotto dall''applicazione al momento della costruzione di un tenant: nessuna sorgente legacy esiste ne'' puo'' esistere, perche'' descrive righe che il legacy non ha mai avuto. Registrata QUI e non accanto alla 000319, per la stessa ragione delle due voci sopra: questo file esegue l''asserzione "0 UNCLASSIFIED", quindi nessuna migrazione di numero maggiore farebbe in tempo a classificarla. Intercettata da ci-rehearsal.sh alla SECONDA passata, che e'' esattamente il caso per cui esiste — in locale, applicata una volta sola, non si sarebbe vista.')
