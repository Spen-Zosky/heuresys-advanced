...
- **Crosswalk** legacy→v5: users via the existing `sys_users.user_external_code = 'LEGACY:'||legacy.users.id`; newly-created rows (OUs, positions) store their legacy id in `*_metadata->>'legacy_*_id'` and resolve parents/links via that key in a second pass.
