...
| I12 | **Brownfield rule:** old DBMS is enrichment source only; v5 architecture wins on every conflict; every brownfield‑sourced canonical record carries a `sys.sys_source_lineage_records` row. | `BROWNFIELD_IMPORT_STRATEGY.md` |
...
| 5.0.7 | Seed `RTL_BANK_REFERENCE` (158 deterministic Faker‑generated synthetic users, 5 branches, 25 branch positions, flagged) via `db/scripts/seed-reference-bank.ts`. **This is the dev/test fixture, separate from the brownfield import.** The brownfield import (post‑MVP) brings ~270 demo employees across 4 legacy tenants (RTL Bank 158, SmartFood 82, EcoNova 26, Heuresys System 4) and can coexist with the seed under distinct tenant codes. Legacy data is demo (no real PII) generated by the platform owner; anonymization is therefore not required, only `user_is_synthetic = true` tagging. | Reference tenant exists, synthetic flag set on every row |
...
| R7 | 158 synthetic users plausible distribution | Deterministic Faker seed in `seed-reference-bank.ts`; `user_is_synthetic = true` |
...
| RD‑03 | Brownfield scope | 4 tenant in legacy (RTL Bank 158, SmartFood 82, EcoNova 26, Heuresys System 4); 270 employees / 274 users imported with `user_is_synthetic = true` tagging |
