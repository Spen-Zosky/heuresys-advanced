...
| **P4** | Finestra sull'attività: coda del diario di ogni lavoratore, nel battito | io | **FATTO** — a video: w1 e w2, ultime 14 azioni ciascuno, con orario, strumento e comando. Le azioni su file portano `oggetto` e non `comando`: senza gestirlo le righe apparivano **vuote**, visto a video e corretto | **fatto** |
| **P5** | Composizione dei cluster: quali file tocca ognuno | io | **FATTO** — a video i perimetri reali: `Z-230 → docs/github/dependabot-triage-2026-05-26.md`, `Z-112 → apps/api/test, apps/api/vitest.config.ts`, più esclusi e 74 sequenziali | **fatto** |
| **P7** | Cockpit: la fascia «in volo adesso», che si muove mentre guardi | io | **FATTO** — driver vivo/fermo, **«ultima azione 2h 26m fa»** con indicatore in pausa quando nessuno si muove, numero di lavoratori con diario. Il lock dice che un driver *esiste*, non che stia *facendo* qualcosa: il diario fermo con lock vivo è il caso che va visto subito | **fatto** |
| **P6** | Configurazione completa, **sempre verificata** | io | **FATTO** — 10 campi modificabili (elenco esplicito), 4 dichiarati intoccabili con il motivo a video. Ogni scrittura tocca **una riga sola** (provato: `git diff` = 1 aggiunta 1 tolta), passa da `zp_state.py verifica` **e** da una verifica di coerenza fra campi, e in caso di no il file **torna com'era** — provato con un caso vero: `majority_to_dismiss=5` con `reviewers=3` respinto e file intatto | **fatto** |
