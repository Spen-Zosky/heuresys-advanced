...
| Source domain | Source tables (TRANSFORM) | Target canonical |
|---------------|---------------------------|------------------|
| **RBP·DGOV (person)** | **`employees`** (the business entity — keyed on `employees.id` / email; `user_is_synthetic = true`) | **`sys.sys_users`** (identity) + `sys.sys_user_*` satellites |
...
1. **Iterate over `employees`** (not `users`): every imported person row (one per `employees` row) receives `user_external_code = 'LEGACY_EMP::' || employees.id`, `user_is_synthetic = true`, and `user_type = 'SYNTHETIC_REFERENCE'`.
