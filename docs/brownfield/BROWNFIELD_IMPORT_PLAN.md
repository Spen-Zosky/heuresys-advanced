...
**Goal:** import demo person data (users, assignments, skill evidence, learning evidence, KPI evidence) from the 4 legacy tenants (RTL Bank 158, SmartFood 82, EcoNova 26, Heuresys System 4 = **270 employees / 274 users total**).
**Nature of data:** the legacy `heuresys_platform` contains **demo data generated by the platform owner** for development. **No real PII**. Anonymization is therefore **not required**; rows are imported as‑is and tagged with `user_is_synthetic = true`.
...
| Source domain | Source tables (TRANSFORM) | Target canonical |
|---------------|---------------------------|------------------|
| RBP·DGOV | `users` (direct import + `user_is_synthetic = true`) | `sys.sys_users` + auth foundation (credentials regenerated — legacy hashes not imported) |
...
**Total Wave 3 source tables:** ≈ 50 (TRANSFORM with `user_is_synthetic = true` tagging — no anonymization, legacy data is demo).

### 5.2 Demo Data Import Protocol (no anonymization)

Since the legacy data is demo (generated by the platform owner, no real PII), import is straightforward:

1. **Tag**: every imported user row receives `user_is_synthetic = true` and `user_type = 'SYNTHETIC_REFERENCE'`.
2. **Email domains**: legacy domains (`rtl-bank.org`, `smartfood.org`, `econova.org`, `heuresys.com`) are kept as‑is — they are demo addresses, not real corporate domains. If duplicate emails arise across tenants, append a tenant suffix (`user@rtl-bank.org#RTL_BANK`) only when needed to satisfy the `(tenant, lower(email))` uniqueness constraint.
3. **Tenant resolution**: `users.employee_id → employees_core.tenant_id` join is performed; the 2 platform users with `employee_id IS NULL` are attached to the `Heuresys System` tenant (or downgraded to PLATFORM_ADMIN scope, see decision in §7.5).
4. **Password hashes**: legacy hashes are **not imported** (different algorithm, possibly weak). New `sys.sys_auth_credentials` rows are generated with a placeholder hash flagged `must_rotate = true`; every user is forced to reset on first login.
...
1. `user_is_synthetic = true` on every imported user row.
2. `user_type = 'SYNTHETIC_REFERENCE'` consistency (cross‑checked by `sys.v_synthetic_user_flag_consistency` returning 0).
...
```sql
-- Every Wave 3 user is synthetic
...
WHERE user_id IN (SELECT target_record_id FROM sys.sys_source_lineage_records
  ...
  AND (user_is_synthetic = false OR user_type <> 'SYNTHETIC_REFERENCE');
-- Expected: 0
...
3. Delete `sys.sys_users` rows where `user_is_synthetic = true` AND linked to Wave 3 lineage AND no remaining FK references.
