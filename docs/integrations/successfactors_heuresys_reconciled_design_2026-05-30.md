...
status: EXPLORATORY / NOT-APPROVED — riconciliato con la SoT reale (docs/kb/SOT_STATE.md)
date: 2026-05-30
...
> [!danger] Due conflitti invariante da decidere PRIMA (regola §9 "fermarsi e chiedere")
> - **I12 (brownfield = enrichment no-PII, dati demo).** L'import SF live porta **PII reale** di dipendenti veri. Riusare le *tabelle/pattern* brownfield è ok, ma la *governance* I12 "no-PII" **non si applica** a questa sorgente → serve una decisione esplicita di scoping/GDPR (vedi §8.2).
...
5. **Decisione di governance PII/GDPR** che solleva I12 per questa sorgente (non è codice, è ADR).
...
| **I12** brownfield = enrichment **no-PII** | 🔴 **conflitto** | SF live = PII reale (vedi §8.2) |
...
### §8.2 — Conflitto I12 (PII/GDPR) — il nodo vero
Il brownfield è dichiarato **no-PII, dati demo** (I12). L'import SuccessFactors di un cliente banca porta **dati personali reali** (nome, email, eventuale DOB/national ID/retribuzione). Riusare *tabelle e pattern* brownfield è legittimo, ma **la policy I12 non copre questo caso**. Serve una decisione esplicita prima di procedere:
- definire `column_mapping_pii_disposition` per ogni campo PII (NONE/PSEUDONYMIZE/MASK/DROP/TAG_SYNTHETIC — il campo esiste già);
- decidere se l'import SF gira solo su tenant reali (non sul `RTL_BANK_REFERENCE` sintetico);
- valutare un ADR "SF live import governance" che scope-limita I12 a brownfield-legacy ed estende la policy PII alla sorgente SF.
...
1. **PII/GDPR (I12)** — bloccante: come scope-limitare I12 ed estendere la policy PII alla sorgente SF? (§8.2)
