...
## ▶ RESUME
P0+P1 done (backup 416 MB; 2 extra tenants deleted → HEURESYS + RTL_BANK only). Legacy VM source confirmed. **P2 in exploration** (findings above). **NEXT = design the P2 mapping strategy** (legacy job catalog × advanced hierarchy × per-incumbent legacy job, email-matched) → present to Enzo → then migration/seed + re-test. P2 is the blocker for R2 (role assignment needs correct titles). Skills (P3): 31 used have ESCO-resolved names → readable codes derivable; comp (8 bands) already clean. Execution is multi-phase; each destructive phase needs backup + idempotent script + re-validation. R2 (roles) is blocked on P2 (correct titles). Cross-ref: `docs/kb/RBAC_UIX_PERSPECTIVES_PLAN.md` (R2), `memory/project_rtl_tenant_rebuild.md` (S950 rebuild), ADR-0023 (no-PII legacy source doctrine).
