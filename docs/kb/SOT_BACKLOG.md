...
| **B-44** | ✅ **FATTO** 2026-05-28 | cross-OS idempotent bootstrap: `scripts/vm-bootstrap.sh` (Linux server, systemd, public 8013/3013) + `scripts/dev-bootstrap.sh` (Mac/Linux-desktop) + `scripts/dev-bootstrap.ps1` (Windows) + `deploy/README.md`; central VM DB via tunnel 5433. Spec/plan in `docs/superpowers/{specs,plans}/2026-05-28-cross-os-bootstrap*`. **Verificato live** su arm64 VM + Windows; Mac DEFERITO (offline); amd64 by-construction (lockfile cross-platform). Side-fix: root scripts `--filter` single→double quote (Windows cmd) | Mac live-verify quando online | 
