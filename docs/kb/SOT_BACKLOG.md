...
- **Data-reconciliation classificato + ready-to-run**: nuovo doc autoritativo **`docs/kb/DATA_RECONCILIATION_PLAN.md`** — 67 tabelle vuote in cat(i) ~15-20 deterministiche/eseguibili-per-card, cat(ii) ~20-25 derived/needs-decision, cat(iii) ~20-25 scaffold/no-source. **`sys_kpi_definitions`** (da `process_kpis`, 81 righe, 1:1) = #1 unblock, card design completo nel plan, sblocca a cascata `sys_process_kpi_templates` + cluster KPI. **Esecuzione GATED** su run production supervisionato (VM, idempotente, backup-guarded): infra locale non import-ready (legacy_mirror=0, staging vuote, dump assenti). Vedi plan §3 per i comandi. **Greenlight Enzo richiesto** per il run (DECISION AUTHORITY — production write batch).
