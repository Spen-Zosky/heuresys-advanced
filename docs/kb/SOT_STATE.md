...
- **Counts verified (real count(\*), post-ANALYZE 2026-06-01)**: **sys_users 161**, **positions 162**, **org_units 26**, **assignments 160 PRIMARY**, **2 tenant ACTIVE** (RTL_BANK 158 + HEURESYS 3 — case-study scope), **sys.* populated 65/134**. Lineage: **69.450** rows. Brownfield registry: 93 source_tables, 1164 source_columns, 94 table_mappings, **1271 column_mappings**, 18 staging.wave1_*, tenant_id_mappings 2 (in-scope). import_runs: 21 COMPLETED + 1 CANCELLED + 1 FAILED. **49 migration** (000048 = B-51 titles, 000049 = R2 role grants). **job_roles 227** (+25 `RTL-ROLE-*` da B-51, S955); **162 positions** con `position_title` professione reale + `position_job_role_id` wired 162/162. **RBAC R2 ✅**: i 4 ruoli holderless (PROCESS_OWNER/BLUEPRINT_MANAGER/READ_ONLY/CEO) ora assegnati a veri utenti RTL per funzione + login (vedi `RBAC_UIX_PERSPECTIVES_PLAN.md`).
