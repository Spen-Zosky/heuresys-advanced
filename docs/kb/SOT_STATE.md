...
- **57 file migration** idempotenti `000001..000058` (**gap `000035` cosmetico**, documentato). Ultime: `000054` teams/roles (R1b) · `000055` team UI interface · `000056` reclassify activity_classification_mappings (ADR-0025 §5.4) · `000057` analytics:view permission (BI P1) · **`000058` reconciliation registry — `sys.sys_reconciliation_registry` + vista `sys.v_reconciliation_status` (S960, F1 del reconciliation-closure cycle; status live query-abile per ogni tabella sys, 0 UNCLASSIFIED)**. Contract: `pnpm db:migrate` ×2 → pg_dump diff vuoto + `db:validate` 7/7 (verificato S957; 000056 idempotente ×2 S958).
