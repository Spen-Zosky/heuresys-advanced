...
> ⚠️ **EMPLOYEE-CENTRIC ingestion correction 2026-06-01 (S954, ADR-0024 / I14).** Le metodiche di popolamento dal SoT legacy erano **user-centric** (key `LEGACY:<users.id>`, `users → sys_users`); la realtà strutturale è **employee-centric** (207 FK → `employees` vs 45 → `users`). Corretto: `sys_user*` ⟸ legacy `employee*`, `users` → solo `sys_auth_*`, key `LEGACY_EMP::<employees.id>`. Fase 1 (doc) ✅; Fase 2 (re-key 5 seed `rtl-rebuild`) + Fase 3 (re-key live, gated) in corso. Dottrina: `docs/brownfield/EMPLOYEE_CENTRIC_MAPPING_DOCTRINE.md`; debito D-13; backlog B-50(e). **I dati S950 sul bare-metal sono persone corrette ma key sbagliata** (`LEGACY:<users.id>`) — il re-key è Fase 3.
