...
- **54 file migration** idempotenti `000001..000055` (**gap `000035` cosmetico**, documentato). Ultime: `000051` skill_categories nullable FK (ADR-0025) · `000052` activity-class kind constant · `000053` `sys_user_preferences` (P1) · `000054` teams/roles (R1b) · `000055` team UI interface. Contract: `pnpm db:migrate` ×2 → pg_dump diff vuoto + `db:validate` 7/7 (verificato S957).
...
- **Counts verified (real count(\*), post-ANALYZE 2026-06-01)**: **sys_users 161**, **positions 162**, **org_units 26**, **assignments 160 PRIMARY**, **2 tenant ACTIVE** (RTL_BANK 158 + HEURESYS 3 — case-study scope), **sys.* populated 65/134**. Lineage: **69.450** rows. Brownfield registry: 93 source_tables, 1164 source_columns, 94 table_mappings, **1271 column_mappings**, 18 staging.wave1_*, tenant_id_mappings 2 (in-scope). import_runs: 21 COMPLETED + 1 CANCELLED + 1 FAILED. **54 file migration** (000048 B-51 titles, 000049 R2 role grants, 000050 U1 ui-interfaces, 000051 skill_categories nullable, 000053 sys_user_preferences, 000054 teams, 000055 team-ui). **job_roles 227** (+25 `RTL-ROLE-*` da B-51, S955); **162 positions** titolate+wired 162/162. **RBAC R2 ✅** (4 ruoli holderless assegnati per funzione + login) + **U1 ✅** (`sys_ui_interfaces` 23 interfacce + `GET /v1/me/interfaces`, hybrid gate, test 5/5) + **U2 ✅** (`(authenticated)/layout.tsx` sidebar DB-driven da `/v1/me/interfaces` via hook `useMyInterfaces` + filtro perspective PET, gating ora server-side, E2E 76/76 PROD). Vedi `RBAC_UIX_PERSPECTIVES_PLAN.md`.
