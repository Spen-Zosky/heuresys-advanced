...
- **Conteggi live**: 26 OU (RTL_BANK 23 + HEURESYS 3), 162 pos, 844 pos-skill-req, 172 pos-kpi-req, 1791 pos-learning-req, 126051 occ-skill-req, 902 skill-evidence, 160 PRIMARY-active assignments, 2 tenant ACTIVE.
- ⚠️ **GAP CRITICO PESO**: `position_economic_weight` è **NULL su tutte le 162 posizioni**; `position_criticality` popolato ma skew (160 MEDIUM / 2 CRITICAL). → la pesatura va con **COALESCE(economic_weight, criticality_factor, 1.0)**, auto-upgrade quando i pesi verranno popolati.
