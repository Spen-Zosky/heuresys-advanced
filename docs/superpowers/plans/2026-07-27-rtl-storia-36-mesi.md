...
- [x] **Step 12.1: batteria completa** — **FATTO S1035** (batteria intera VERDE con 119 selftest, dossier 206/206, `pnpm db:validate` riportato a verde correggendo 6 asserzioni-fotografia dentro le migration). Ri-verificato 2026-07-29 dopo le riparazioni del 12.2: **custodia VERDE** con i due check nuovi.
      *(criterio originale: `verify-storia36.sql` intero VERDE self-test inclusi + TUTTI i dossier di `verify-storia36-dossier.sql` — persona 162/162, processi, OU, posizioni, team, cascata KPI, tenant, con il check di completezza 206/206 tabelle mappate — + riconciliazione aggregati API vs righe + `pnpm db:validate` + le 6 viste = 0.)*
- [x] **Step 12.2: audit semantico su TUTTE le 206 tabelle** — **FATTO 2026-07-29**. Strumento ri-eseguibile `db/scripts/audit-storia36-semantic.py`: perimetro e applicabilità delle regole **derivati dal catalogo di sistema** (AP-03 — nessun elenco a mano, una tabella nuova entra da sola), 10 regole, 604.098 righe coperte. Esiti a tre vie, tutti espliciti: **riparato** (`db/seeds/storia36/12_registration_dates.sql` — 4.847 righe ridatate + 178 cicli di straordinario chiusi + 307 azioni su lacune mature prese in carico + 143/117 date di studio riportate all'anno accademico; twice-run 0 su tre corse), **spiegato** (`db/scripts/audit-storia36-explanations.txt` — classi legittime con motivo misurato, non asserito), **dichiarato** (`db/scripts/audit-storia36-empty-tables.txt` — 10 tabelle vuote, ognuna col perché). Due check nuovi permanenti (**C12a** datazione + completezza del perimetro, **C12b** code di approvazione) con 3 selftest; un mio difetto di misura (fiducia in `pg_stat_user_tables`) scoperto dal selftest e corretto. Verbale: `docs/kb/storia36/AUDIT_FINALE.md`.
- [x] **Step 12.3: rete di sicurezza intera** — **FATTO 2026-07-29**. Vitest: **1.517 test verdi / 0 falliti** (i 2 file inizialmente rossi erano saturazione del tunnel — due suite lanciate in parallelo per errore mio, deadlock su `sys_auth_mfa_factors`; rilanciati da soli: 15/15). Playwright `test:e2e:prod:node22`: **254 passed / 0 failed**. Guardia attori **9/9**. Due prove che **fotografavano lo stato** trovate e riscritte come invarianti (commit `11ef27d4`): `/me/approvals` pretendeva l'empty-state (il C7 ha popolato le richieste) e `surveys-m2` cercava la survey «Q4» per nome (il C8 ha rinnovato i cicli e quella si è chiusa) — stessa classe vietata da `feedback_no_hardcoded_test_data`.
- [x] **Step 12.4: demo live** — **FATTO 2026-07-29** (commit `32a3286d`). Percorso investitore come spec **ripetibile** (`apps/web/tests/e2e/storia36-demo.spec.ts`, gated `STORIA36_DEMO=1`, fuori dalla suite di regressione): cruscotto → andamento presenze sui 36 mesi → elenco e scheda persona → coda approvazioni → clima → retribuzioni → portale della persona. 9 schermate in `qa_artifacts/storia36/demo/`. **Ha trovato un difetto che nessun test vedeva**: l'intestazione mostrava `roles[0]` e presentava `federica.marchetti` (TENANT_ADMIN) come «**Dipendente**» su ogni pagina → `apps/web/src/lib/role-precedence.ts` (fonte unica, usata anche da SystemHealthLive che teneva una lista parziale) + regressione `role-badge.spec.ts` che deriva l'atteso dai ruoli VERI via API. **Due falsi indizi evitati guardando gli artefatti**: «48,25 utenti» era l'animazione colta a un terzo; la scheda persona era stata fotografata mentre diceva «Caricamento…». **Rilievo aperto registrato (#81)**: `/users/[id]` mostra solo anagrafica tecnica (UUID grezzi, fuso, data ISO) e nulla dei 36 mesi — i dati ci sono, è la scheda che non li compone.
      *(criterio originale: login federica.marchetti → dashboard con trend 36 mesi → una persona con storia completa → inbox approvazioni → engagement, con schermate.)*
- [x] **Step 12.5: manutenzione ecosistema** — **FATTO 2026-07-30**. Deploy VM **OK** e verificato live (`https://www.heuresys.com/login` 200 · `/api/readyz` ready). Al primo giro il **cancello CI** ha fermato il deploy per timeout a 900s (il commit toccava il lockfile → suite integration ripartita intera): rilanciato con `CI_GATE_WAIT=2700`. **Ordine dei target ora è regola** (Enzo, S1036): VM per prima e fino in fondo, poi il linux-pc — la VM è la vetrina su cui Enzo prova di persona. Clone DB del gemello rigenerato (`clone-vm-db.sh`, row-counts verificati vs VM: 163 utenti / 181 posizioni / 116.015 presenze) + **`heuresys_ci` ricostruito** dalla base fresca (`setup-ci-database.sh`, parità 262 tabelle / 6 estensioni). Deploy linux-pc **OK**.
- [x] **Step 12.5b: custodia schedulata** — **FATTO 2026-07-30**, e **provata in entrambi i versi**. Unità + timer (`deploy/systemd/heuresys-advanced-storia36-custodia.{service,timer}`, lunedì 04:30, `OnFailure=heuresys-unit-failure@`) installate e `enabled` su **VM e linux-pc** (prossimo scatto: lun 3 agosto). **Caso positivo**: esecuzione reale sulla VM → `custodia VERDE` in 11s, `Result=success`. **Caso negativo eseguito**: fallimento iniettato con finestra impossibile (`--window-end=2023-01-01`) → il registro degli allarmi, **vuoto prima della prova**, contiene ora `{"ts":"2026-07-29T22:57:13Z","unit":"storia36-custodia-prova-guasto","result":"exit-code","exit":"1"}` e il journal a priorità `err` la riporta. Un presidio che non sa segnalare non è un presidio.
      *(criterio originale: rigenera `heuresys_ci` dal clone PROD + `align-clones` con deploy + CI verde su main.)*
      *(criterio originale: timer settimanale che esegue `storia36.sh custodia` con `OnFailure`, dopo il refresh del clone; prova con una violazione iniettata visibile nel registro.)*
- [x] **Step 12.6: skill `storia36-custodia`** — **FATTO 2026-07-31**, e solo dopo aver soddisfatto la precondizione che il piano stesso poneva. Il modo **`avanzamento` non esisteva** (falliva esplicito, exit 2): è stato implementato (`db/seeds/storia36/13_avanzamento.sql`, finestra mobile, festività CALCOLATE con Meeus/Butcher perché una lista scritta a mano si ferma al primo anno nuovo) ed **esercitato per davvero** — la storia era invecchiata di una settimana, estesa dal 24 al 30 luglio: 624 presenze, 2 richieste di ferie, 1 rinnovo di abilitazione, terza corsa 0 righe. La custodia ha prodotto **4 rossi tutti triagati** (2 difetti miei, 1 lacuna vera dell'avanzamento, 1 saldo derivato non riallineato), quindi la skill codifica un procedimento **esercitato**, non sperato. Skill in `.claude/skills/storia36-custodia/SKILL.md`: perimetro solo ricorrente (custodia · avanzamento · triage a tre esiti), la costruzione esclusa; punta a `storia36.sh`, `DOSSIER_REGISTRY.md` e ai doc di dominio senza duplicarli (AP-01); ogni path citato verificato esistente. Nuovo check permanente **C12c** (calendario denso, coerente, e festività calcolate che COINCIDONO con quelle scritte a mano) con 3 selftest.
      *(criterio originale: DOPO che primo run e ripetibilità sono provati, mai prima — la skill codifica il procedimento ESERCITATO, non quello sperato). Scriverla con `superpowers:writing-skills`. Perimetro: SOLO ciò che ricorre — custodia, avanzamento, triage a tre esiti — MAI la costruzione (una tantum, resta nel piano che si archivia). Trigger: "custodia storia36" · "verifica la storia" · "avanza la storia" · "la demo è rotta". Regola anti-drift AP-01: la skill PUNTA a `storia36.sh`, `DOSSIER_REGISTRY.md` e ai doc di dominio, non li duplica. Precondizione di scrittura: almeno UNA custodia reale eseguita con almeno un rilievo triagato, e un avanzamento mensile riuscito.
- [x] **Step 12.7: chiusura** — **FATTO 2026-07-31**. PROGRESS tutto spuntato (13/13 cluster), AUDIT_FINALE.md aggiornato con il terzo modo, register #77/#80 → DONE al handoff. Il programma è chiuso: la storia non è più un lavoro in corso, è un **dataset presidiato** — custodia settimanale schedulata su VM e linux-pc (provata nei due versi), `avanzamento` che la porta a ieri quando serve, e una skill che codifica il procedimento per chi la manterrà. Coda registrata a parte, NON parte del C12: #81 (scheda persona) nato dalla dimostrazione 12.4 e chiuso lo stesso giorno; difetto del seeder personas pre-Z-262 trovato e riparato durante il lavoro (commit `baed0bb2`).
