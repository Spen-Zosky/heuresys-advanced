...
3b. **RBAC → web-interface access logic, ported from legacy** (Enzo): the legacy DB has `rbp_roles / rbp_role_permissions / role_permissions / critical_roles` defining role→permission→UI access. Extract that logic and **reproduce it** in heuresys-advanced — which already has RBAC (8 roles × 394 mappings in `sys.sys_auth_role_permissions` + per-route `requirePermission` + sidebar role-gating). Diff the two; bring over the legacy richness (e.g. per-role menu/page visibility), **substituting legacy routes/pages with the current heuresys-advanced ones** (`/dashboard`, `/positions`, `/compensation-intelligence`, `/organization/org-chart`, `/me/*`, `/admin/*`…). Goal: each real user sees exactly the interfaces their role grants.
3c. **Re-wire E2E personas to REAL users** (NOT preserve `*.test`): map the 5 personas (platformAdmin/tenantAdmin/manager/employee/outsider) onto real rtl-bank.org/heuresys.com users by RBAC role (TENANT_OWNER→tenantAdmin, DEPT_HEAD→manager, EMPLOYEE→employee, heuresys.com→platformAdmin); set a known test password on those accounts; update `apps/web/tests/e2e/fixtures.ts` + `auth.setup.ts` to the new addresses. Then `rtl-bank.test` is deleted with the rest.
...
- **E2E personas — RE-WIRED, not preserved** (Enzo's correction): `rtl-bank.test` **goes away** under the 2-tenant model. The true invariant is **CI stays green**, achieved by remapping the 5 personas onto REAL users by role (step 3c), not by keeping `*.test` accounts. Sequence carefully: re-wire fixtures + set test passwords BEFORE deleting `rtl-bank.test`, so no E2E run is left without valid logins.
