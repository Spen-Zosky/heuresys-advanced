...
> **Status**: DESIGN — DECISIONS RESOLVED (S972/S973, 2026-06-07). Capability ④/5 of the platform-capabilities program (`2026-06-03-platform-capabilities-roadmap.md`). **No code until this spec is approved + a plan is written.** All eight design decisions are now resolved with best-practice defaults (§10) — the spec is **implementation-ready, no longer gated on input**. Approval is Enzo's (the program gate, now a go/no-go) — this spec closes the DESIGN stage only.
...
- **Read surface**: the admin SPA reads through `/v1/content/*` (tenant-scoped, all statuses by permission). The ESS knowledge-base read is a **dedicated self-scope `/v1/me/content` path in the `me` module** (DECIDED D-7, ADR-0011) with a `published`-only filter enforced server-side by scope (ESS personas cannot see drafts and never touch admin permissions). This ESS surface lands at P2 (see §7), not P1.
...
| **3** | Rich editor + media/attachments | rich-text `@heuresys/ui` primitive + asset refs | external object-store URI refs (DECIDED D-2 — no PG blobs) |
...
| DELETE | `/v1/content/:id` | `content:delete` | ✅ | DECIDED D-5: soft-delete (`status='archived'`) for published content; hard delete (CASCADE versions) only of drafts |
...
| GET | `/v1/me/content` | `me:content:read` | — | **ESS** self-scope read (`me` module, ADR-0011), published-only, server-side filtered — P2 (DECIDED D-7) |
| GET | `/v1/me/content/:id` | `me:content:read` | — | **ESS** read one published article — P2 |

**Permissions** (seed migration mirroring `000078`): the 5 admin permissions `content:read`, `content:create`, `content:update`, `content:delete`, `content:publish` — plus 1 self-scope `me:content:read` for ESS (DECIDED D-7, lands with the P2 ESS surface). Audience by precedent: `content:read` → the 6 non-leaf HRMS-read roles (`PLATFORM_ADMIN, TENANT_ADMIN, BLUEPRINT_MANAGER, HRMS_MANAGER, PROCESS_OWNER, MANAGER`); writes (`create/update/delete`) → `PLATFORM_ADMIN, TENANT_ADMIN, HRMS_MANAGER`; **`content:publish` → `PLATFORM_ADMIN, TENANT_ADMIN, HRMS_MANAGER`** (DECIDED D-6: distinct permission from create/update so author ≠ publisher, granted to `TENANT_ADMIN` + `HRMS_MANAGER` so HR owns policy/handbook publication). **`me:content:read`** → `USER`/`READ_ONLY` (and inherited by elevated roles), filtered server-side to `published` only (DECIDED D-7 — dedicated `me` module per ADR-0011, never bolted onto admin routes). `ON CONFLICT DO NOTHING`, idempotent, post-condition assert count, NB PLATFORM_ADMIN listed explicitly (not auto-granted — the `000005` grant was one-time).

## 6. Frontend

- **Admin authoring** (`apps/web/src/app/(authenticated)/content/*`): list/table (TanStack Query over `/v1/content`), create/edit form, version-history drawer, publish/unpublish actions, category management. Composed from `@heuresys/ui` primitives (`DataTable`, `Button`, `Card`, form inputs) + the **editor primitive** (DECIDED D-1: plain `<textarea>` markdown in P1, rich-text primitive in P3). i18n IT+EN keys (`pnpm i18n:check` parity).
- **ESS knowledge base** (ESS route group, e.g. `/me/knowledge` per ADR-0011): read-only published content, category filter, search. **No `/me/*` route added to existing modules** — the self-scope content read gets a route in the dedicated `me` module (DECIDED D-7, ADR-0011), reading `/v1/me/content`. Published-only enforced server-side.
- **Editor primitive — upstream**: the editor component is a **`@heuresys/ui` primitive added in `ux-design-shared`**, released to npm, then bumped here (post-X18 workflow). It is **NOT** created in `apps/web`. DECIDED D-1: **P1 ships a plain `<textarea>` markdown source** (safe sanitized render, no new primitive) to unblock authoring with zero upstream dependency; the **rich-text primitive lands at P3**.
- **Live-data E2E doctrine** (`NEXT_SESSION_MVP_2A`): every page fed by a real `/v1/content/*` call; no mock/fixture/`initialData`. No page commit without a Playwright E2E green that logs in as a real seeded persona (`admin@heuresys.com` / `<TEST_ADMIN_PASSWORD>` or HR persona), authors → publishes → re-reads, and an ESS persona reads the published article. Empty-state UI only for a real empty API response.

## 7. Phasing

| Phase | Scope | Effort | Risk |
|---|---|---|---|
| **P1** | Minimal read + author: `sys_content_documents` + `sys_content_categories` (versions table created but single active version), `content` module CRUD (read/create/update/delete + category CRUD), 5 admin permissions, admin authoring page (markdown `<textarea>`), admin KB read, integration + E2E green, registry registration. **No publish workflow, no ESS surface yet.** | **M** (~1.5 sessions; module mirrors `surveys` 1:1) | low — established pattern; editor scope already bounded by DECIDED D-1 (markdown only) |
| **P2** | Versioning (history + restore over `sys_content_versions`) + publish/lifecycle workflow (`draft→in_review→published→archived`, `content:publish`, separation of duties per DECIDED D-6) + ESS knowledge-base read surface (`/v1/me/content`, `me:content:read`, published-only, self-scope per DECIDED D-7 / ADR-0011). | **M** (~1–1.5 sessions) | med — lifecycle transition correctness + ESS scope isolation (tested) |
| **P3** | Richer editor primitive (rich-text in `@heuresys/ui`, DECIDED D-1) + optional media/attachments via external object-store URI refs (DECIDED D-2 — no PG blobs) + BPM/process-doc cross-link (`document_linked_process_id`, DECIDED D-8) + full-text search (`pg_trgm`/`tsvector`). | **M–L** (multi-session, each sub-slice independent) | med — media object-store wiring is the main new work; editor primitive is upstream `ux-design-shared` work |
...
- **Media/attachments** (P3) has no storage layer today (the platform stores `sys_user_documents` as URIs, not blobs). DECIDED D-2: if media is added at all it is **external object-store URI references, never PG blobs** — explicitly deferred out of P1/P2. The only open *implementation* detail (which object store) is a P3 plan-step choice, not a blocker for P1/P2.

## 10. Resolved design decisions (S972)

All eight decisions that were previously gated on Enzo's input are **now resolved** with best-practice defaults (the CLI recommendations + standard practice), so this spec is **implementation-ready** — no longer blocked waiting for input. Each row records the **DECIDED** choice and a short justification. These are technical/design calls; none was irreducibly human (see §"Residual human sign-off" below).

| # | Decision | DECIDED | Best-practice justification |
|---|---|---|---|
| **D-1** | **Editor format / primitive** | **Markdown in P1** (`<textarea>` source + safe markdown render); **rich-text WYSIWYG primitive in P3** as a new `@heuresys/ui` primitive added upstream in `ux-design-shared`. `document_body_format` CHECK already accommodates `'markdown'`/`'html'`. | Ship value early with zero upstream dependency; defer rich-editor complexity and its XSS surface until the foundation is proven. Markdown is rendered through a safe (sanitized) renderer — no raw HTML injection in P1. |
| **D-2** | **Media / attachment storage** | **No blob storage in the DB.** P1/P2 are **text + markdown only**; P3 optionally adds **URI references to an external object store** — **never** `bytea`/large-object blobs in PG. | Keep Postgres lean: binary bytes do not belong in the row store (bloats the DB, slows backups/replication). The platform already stores `sys_user_documents` as URIs, not blobs — CMS follows the same discipline if/when media is added. |
| **D-3** | **Reuse/extend `sys_user_documents` vs new tables** | **New `sys.sys_content_*` tables.** Reusing/extending `sys_user_documents` is **rejected**. | The existing table is person-owned `ON DELETE CASCADE`, unversioned, untaxonomized, self-scoped private files (657 live rows). Bending it into tenant-owned versioned published content would corrupt its semantics and put the 657 rows at risk. The two domains are disjoint — a clean new schema protects both. |
| **D-4** | **Taxonomy shape** | **Flat tenant categories in P1**, with a **reserved nullable `category_parent_id` self-FK** for a future tree (no recursion shipped in P1). Tags remain a possible P3 add. | Avoid premature hierarchy: a flat taxonomy covers the P1 knowledge-base/policy use cases without recursion scope creep, while the reserved FK keeps a tree migration cheap and non-breaking later. |
| **D-5** | **Delete semantics** | **Soft-delete for published content** (`document_status='archived'`, RD-08 `varchar + CHECK`) **+ audit — never hard-delete published content**. **Hard delete only of drafts** (CASCADE versions). | Content is a record: once published it has reach and history that must be preserved (audit, who-saw-what, restore). Drafts have no such footprint, so a true hard delete of a never-published draft is safe and keeps the table tidy. |
| **D-6** | **Publish authority (separation of duties)** | **`content:publish` is a distinct permission** from `content:create`/`content:update` (so author ≠ publisher is possible), granted to **`TENANT_ADMIN` and `HRMS_MANAGER`**. | Separation of duties on the state-changing publish action: an author drafts/edits, a distinct publisher promotes to `published`. Granting it to both `TENANT_ADMIN` and `HRMS_MANAGER` gives HR genuine ownership of policy/handbook publication without diluting the SoD split. |
| **D-7** | **ESS read path** | **Self-scope `me:content:read` via the `me` module** (ADR-0011) — ESS knowledge-base read is a dedicated `/me/*` surface, not bolted onto the admin `/v1/content/*` routes. | Keeps the admin/ESS permission split clean and consistent with ADR-0011 ("ESS routes get a dedicated module, don't bolt `/me/*` onto existing modules"). Published-only filtering stays server-side; ESS personas never touch admin permissions. |
| **D-8** | **BPM/process-doc cross-link** | **Deferred to P3** — an optional nullable FK from content to `blueprint-*`/`process-*` (`document_linked_process_id`); **not in P1**. | It is the roadmap's natural hook ("process documentation tied to the BPM/blueprint engine") but additive and deferrable: a nullable FK + join can land in P3 without reworking the P1 foundation. |

### Residual human sign-off

**None required to proceed to plan/implementation.** Every decision above was a technical/design call and is now resolved — there is no irreducibly human policy choice left open in the CMS design (D-6's SoD policy was the only candidate, and the best-practice default — distinct publish permission for `TENANT_ADMIN`/`HRMS_MANAGER` — is adopted). The spec is **ready for the plan → implementation flow on Enzo's go**: his approval remains the program gate (brainstorm → spec → **Enzo approval** → plan → implementation), but it is now an unblocked go/no-go on a fully-specified design, not a request for missing input.

## 11. Honest note

Full CMS implementation is **multi-session** (P1 ~1.5 sessions, P2 ~1–1.5, P3 multi-slice). This spec closes the **DESIGN stage only** — no code, no migration, no DDL is authored here. All eight design decisions are now **resolved** (§10) with best-practice defaults, so the spec is **implementation-ready and no longer gated on input** — only on Enzo's go/no-go. Per the program model (brainstorm → spec → **Enzo approval** → plan → implementation), the next artifact is a **P1 plan**, authored once Enzo approves this design. CMS is the roadmap's #4 priority (useful, low-risk, ties to BPM; not differentiating) — lower than ① BI / ② AI.
