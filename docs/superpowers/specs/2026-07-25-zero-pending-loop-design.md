...
| V4 | `--permission-mode`, `--allowedTools`, `--max-turns` esistono. **`--max-turns` esce con errore, non graceful** | Il driver tratta l'exit non-zero come «iterazione troncata», non come fallimento del cluster |
| V5 | Il censimento **è già stato eseguito** (S1029, oggi): 497 voci grezze da 10 finder → 248 cluster, 3 verificatori adversarial, esito 497 mappati / 0 persi / 0 inventati. Ondate W0-W6, `done when` osservabile, 34 cluster già chiusi | Il punto 3 diventa **verify + refresh delle sole fonti stale**. Censimento full solo se il piano è assente o oltre soglia, e dietro stima di costo in forma R20 |
| V6 | Ripartizione: **218 cluster autonomi (~924h)** · **30 bloccati su Enzo (~446h)**: 19 decisione-business, 9 esterno, 2 segreto (app-password Outlook, client secret IdP) | «Zero pendenze» non è raggiungibile in autonomia. Condizione primaria riformulata (§1) |
...
| Modo | Quando | Cosa fa |
|---|---|---|
| **bootstrap** | prima chiamata sul progetto, o piano assente/stale oltre soglia | esegue la procedura codificata di avvio sessione (`session_start.py`) · **verifica** il piano S1029 esistente (integrità: 248 cluster, `dependsOn` risolti, checkbox coerenti col register) · refresh delle **sole** fonti stale · ricostruisce la todo tracciabile · dichiara l'incarico compreso e le regole di autonomia con cui opererà |
...
2. **Due verifiche di natura diversa** — non due esecuzioni dello stesso test. `zp_gate.py` **rifiuta** una coppia omogenea. Coppie ammesse:

| Verifica A | Verifica B ammessa |
|---|---|
| integration test su DB reale (tunnel :5433) | prova live con evidenza (comando + output + path + timestamp) |
| test automatico | unit test sul ramo che l'integrazione non raggiunge |
| Playwright E2E con login reale | query psql che conferma la mutazione lato DB |
| migrazione applicata due volte con diff `pg_dump` vuoto | `db:validate` (7 viste) verde |
...
Esiste **un solo ambiente ed è produzione** (V7). Un loop non presidiato che tocca indiscriminatamente `db:migrate`, `vm-deploy.sh` (riavvia i systemd `api`/`web`) e `push` su `main` per centinaia di ore non è autonomia, è roulette. I 248 cluster si classificano **una volta**, in `zp.config.yaml`, e la classe determina la corsia.
...
| **E · bloccato su Enzo** | i 30 cluster di W6 (19 business · 9 esterno · 2 segreto) | **no** | input di Enzo — è il vassoio della condizione primaria |

`--lane safe` esegue A+B. `--lane full` aggiunge C. **D non entra mai in corsia non presidiata, per costruzione dello script** — non per disciplina del modello.
...
Regola non negoziabile: **il giudizio non si delega in giù.** In dubbio → eredita il modello di sessione. Budget per iterazione dichiarato in config; superato l'80% → il motore passa a `close` invece di aprire un nuovo cluster.
...
2. `bootstrap` su piano esistente → **non** ri-censisce; verifica integrità 248 cluster e segnala eventuali `dependsOn` rotti.
...
8. Iterazione troncata da `--max-turns` → classificata `troncata`, il cluster **non** risulta fallito, la successiva riprende dal `resume-from`.
...
**Effort**: ~14h in 4 tranche. **T1** classificazione dei 248 cluster per raggio d'impatto + `zp.config.yaml` (~3h — è il lavoro che rende sicuro tutto il resto). **T2** `SKILL.md` + `references/` (~4h). **T3** i quattro `zp_*.py` + `zero-pending-driver.sh` (~5h). **T4** accettazione §11 (~2h). Regression risk della *costruzione*: **basso** (nessun runtime toccato). Il rischio vero è nell'*esecuzione*, ed è governato da §6.
