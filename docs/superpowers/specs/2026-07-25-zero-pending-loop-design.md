...
**Data**: 2026-07-25 · **Stato (2026-07-26)**: **IMPLEMENTATO — esercizio non presidiato NON autorizzato**
Non è più una bozza, e tenerla marcata così sarebbe l'ennesima affermazione scaduta: l'impianto descritto qui è stato costruito, committato e sottoposto a due review ostili che l'hanno corretto in più punti. Ciò che aspetta una decisione di Enzo **non è l'approvazione del disegno** — è l'autorizzazione a lavorare mentre non guarda nessuno (`meta.autorizzato_non_presidiato: false`, oggi inserito; il driver esce senza aprire sessioni). Dove questo documento diverge da ciò che sta nel repo, **vale il repo**: qui resta il *perché*, non il *come*. Accettazione: 15 controlli automatici verdi, 4 richiedono una prima corsa presidiata.
...
| V6 | **Una parte dei cluster non è chiudibile da un agente**: aspetta una decisione di business, una dipendenza esterna o un segreto (app-password Outlook, client secret IdP). È una proprietà strutturale del piano, non un numero — la ripartizione si rimisura con `python docs/kb/tools/zp_state.py piano`, che è l'unico modo onesto di citarla. Misurata il 2026-07-25: 218 autonomi (~924h) · 30 su Enzo (~446h). **Ri-misurata il 2026-07-26: 183 autonomi (895h) · 30 su Enzo (446,5h)** — 19 decisione-business, 9 esterno, 2 segreto. Gli autonomi calano perché vengono chiusi; i bloccati no, perché solo Enzo li sblocca | «Zero pendenze» non è raggiungibile in autonomia, e la conseguenza **non dipende dai numeri**: vale finché esiste anche un solo cluster che aspetta Enzo — cioè finché quella colonna non è zero. Condizione primaria riformulata (§1) |
...
**Sub-condizione per iterazione** (quella passata a `/goal`, V2): *il cluster corrente ha gate verde ∧ 2 verifiche su livelli diversi registrate ∧ 0 rilievi adversarial aperti ∧ blocco evidenza DoD presente ∧ piano e register aggiornati — or stop after N turns*.
...
**Effort**: ~14h in 4 tranche. **T1** classificazione di tutti i cluster aperti per raggio d'impatto + `zp.config.yaml` (~3h — è il lavoro che rende sicuro tutto il resto). **T2** `SKILL.md` + `references/` (~4h). **T3** i tool `zp_*.py` + `zero-pending-driver.sh` (~5h stimate su quattro tool; ne sono stati costruiti sei — `zp_classify` e `zp_selftest` non erano previsti qui). **T4** accettazione §11 (~2h). Regression risk della *costruzione*: **basso** (nessun runtime toccato). Il rischio vero è nell'*esecuzione*, ed è governato da §6.
