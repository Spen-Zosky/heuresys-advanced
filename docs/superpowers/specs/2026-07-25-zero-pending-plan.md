...
- [x] **Z-112** (1.0h) — Nessun assert di drift post-suite (residui E2E% sul DB condiviso)
  ...
  - *assorbe*: `p100x:QW-F3`
  - ✅ **CHIUSO S1052** (2026-08-10) — **primo cluster chiuso da una corsa presidiata a più lavoratori**. Lavorato dal lavoratore 1 nel suo albero (commit `7eb39abf`: helper `drift-check.ts` 193 righe + test di integrazione 77 + aggancio in `vitest.config.ts`), istruito da gov con **verdetto VERDE** (`.zp/verdetti/w1-Z-112-20260810-090053.json`, **zero rilievi**: perimetro rispettato, typecheck e lint verdi, cancello test verde in 32 minuti, evidenze ammesse), portato in main col merge `b6824e7e`.
