...
- [ ] **Z-260** (1.0h) — **Dossier di contesto per i revisori adversarial** (impianto `zero-pending-loop`): i 3 revisori ri-derivano ognuno per conto proprio lo stesso contesto — grafo FK, seed delle migrazioni, runner, schema — spendendo ~70 query `psql` sul tunnel per cluster, con parallelismo reale **2** (la macchina ha 4 core: cap `min(16, core-2)`). Su Z-257 sono costati ~21 minuti di orologio. Allegare al diff un dossier di fatti già misurati (come si fa già con `.zp/prove/<ID>-diff.txt`) lascia ai revisori la **controprova mirata** invece dell'esplorazione, senza togliere loro nulla: i numeri restano verificabili con una query sola
  - *chiuso quando*: due cluster consecutivi chiudono la review adversarial con meno della metà delle query per revisore rispetto alla mediana attuale, senza perdita di rilievi confermati (confronto sui run-record)
  - *nota*: decisione di Enzo (S1032) — si applica a **impianto fermo**, mai a corsa in volo
- [ ] **Z-258** (3.0h) — L'ambito tenant ha **tre** classi e il codice ne conosce due: 142 tabelle con FK diretta a `sys_tenancies`, **34 transitive** (ambito ereditato dall'aggregato padre, nessuna colonna tenant propria), 29 globali su 206. Sul tier transitivo il filtro esiste solo se il singolo repository si ricorda della join — `sys_team_members` ha 173 righe e zero colonne tenant, e `teams/repository.ts:270` cancella per `team_id + user_id` senza filtro, affidandosi alla guardia a monte. Nessuna derivazione dei tier nel codice: `grep GLOBAL_TABLES|tenantless|TENANT_SCOPED` trova solo commenti. È l'invariante I5 verificata a campione invece che per costruzione
  - *chiuso quando*: un test calcola il tier di ogni tabella dalla topologia e pretende la forma di filtro corrispondente (WHERE diretto · join obbligatoria all'aggregato · nessun filtro), e fallisce se una tabella cambia tier senza che il codice segua
...
- [x] **Z-250** (1.5h) — **Prima corsa presidiata dell'impianto `zero-pending-loop`.** I sei punti di ridisegno sono **chiusi** (`8293553f`, 2026-07-26): la classe si deduce dall'azione del criterio di chiusura e non dalla descrizione, con un pavimento che gli override non possono abbassare — sette cluster che toccavano la produzione sono usciti dalla corsia non presidiata (14 alzati, **zero abbassati**); `class_c_preconditions` è verificato davvero (dump < 24h + host di prova raggiungibile, altrimenti la classe C resta esclusa); il tipo di prova è confrontato col comando e `echo` è rifiutato con qualunque etichetta; il gate ragiona per livelli e ammette `integration+e2e` e `psql+runtime` che la Definition of Done impone, mentre `staticcheck` non conta più come mezza prova; il self-test rileva le 4 regressioni che prima non vedeva; lo stato si scrive in modo atomico e un id duplicato non sblocca più le dipendenze. **Quello che manca non è codice**: è una corsa vera con Enzo presente, su un paio di cluster di classe A, che chiuda anche i 4 test eseguibili solo con una sessione viva — bootstrap che non ri-censisce, freno tirato a metà lavoro, troncamento da budget, frontiere della description
  - *chiuso quando*: `.zp/runs.ndjson` porta almeno una corsa presidiata conclusa con un cluster chiuso e il suo commit, i 4 test `[a mano]` risultano eseguiti, e Enzo ha deciso se `meta.autorizzato_non_presidiato` passa a true
  - *nota*: il freno **non si toglie da solo**, nemmeno a punti chiusi: accendere l'autonomia notturna è una decisione di Enzo e il design è ancora «BOZZA in attesa di approvazione»
  - **S1052 — LA PRIMA CORSA PRESIDIATA È AVVENUTA.** Sbloccata da una decisione di Enzo su un cerchio che si chiudeva su sé stesso: il freno pretendeva Z-250 chiuso, Z-250 pretendeva una corsa, la corsa pretendeva il driver, il driver era fermato dal freno. Il freno si chiama `autorizzato_non_presidiato` e ora governa esattamente quello: la corsia `full-presidiata` non passa da quel cancello, tutte le altre sì (provato nelle due direzioni: `safe` → exit 3, `full-presidiata` → passa).
  - **Esito della corsa** (`--lane full-presidiata --max-iterations 1`, 2026-08-09 22:40→23:09): **due lavoratori in parallelo** su due cluster di classe A (`Z-112` a w1, `Z-221` a w2), **1728 s contro i 3162 s che sarebbero serviti in fila → guadagno 1,83×** (il piano stimava 1,4-1,6×). Costo 16,24 $. Esiti: `cluster-interrupted` con **1 commit** (`7eb39abf`) e `troncato` con 1 file non committato.
  - **I 4 test `[a mano]`**: ① *bootstrap che non ri-censisce* — **superato**, zero censimenti nel log · ② *freno tirato a metà lavoro* — **superato in entrambe le metà**: STOP creato alle 22:43 con i lavoratori in volo, hanno **finito** alle 23:09 invece di essere uccisi, e al rilancio il driver si è fermato con «il freno è tirato» · ③ *troncamento da budget* — **NON PROBANTE**: il tetto ha contenuto la spesa in tutte e tre le corse (10,96/12 · 5,28/12 · 0,95/1) ma il taglio a metà lavoro non è stato osservato — la corsa da 1 $ si è fermata per il `next: stop` lasciato dalla precedente, non per il tetto · ④ *frontiere della description* — **superato 8/8**, inclusi 3 controlli negativi che restano senza pavimento (il caso storico `Z-153` sale a D, un refactor resta senza classe).
  - **Il reperto migliore non è un test**: il lavoratore ha completato `Z-112`, l'ha committato, e si è **rifiutato di dichiararlo chiuso** perché mancavano i tre revisori adversarial e il budget non bastava a farli girare. Ha preferito fermarsi che millantare.
  - **Difetti strutturali emersi correndo, tutti corretti e provati**: il driver si bloccava con il rapporto che scrive lui (`.zp/PROGRESS.md` tracciato ⟹ due corse consecutive impossibili) · il recinto fermava il lavoro **dentro** il perimetro (non seguiva i `cd`: 2 rifiuti su 4 erano falsi) · il rientro contava zero azioni da quando il diario è uscito dagli alberi · la batteria della guardia era rossa (5 test su 5 guardavano il diario nel posto vecchio) · l'istruttoria accusava di «nessun diario» chi ne aveva 135 righe, e il rilievo falso finiva nel verdetto accanto a quelli veri.
  - **PERCHÉ RESTA APERTO** (l'unica voce che manca, dichiarata invece che aggirata): il criterio pretende «una corsa presidiata conclusa **con un cluster chiuso e il suo commit**». La corsa c'è stata e i commit pure, ma **nessuno dei due cluster ha chiuso**: `Z-112` ha ricevuto verdetto ROSSO dall'istruttoria per **1 prova su 2** richieste da ADR-0026 (lavoro completo e valido — perimetro rispettato, typecheck e lint verdi, 1509 test passati — ma le evidenze non bastano), `Z-221` è rimasto con un file non committato. Entrambi i rossi sono **giusti**: il processo ha giudicato, non ha ceduto.
  - **Il rosso sui test era ambientale, e l'ho verificato invece di assumerlo**: 1509 test passati, zero falliti, **un solo file caduto** (`seed-acquisition`) con `Connection terminated due to connection timeout` in `pool.connect()`. Rieseguito da solo: **5/5 verde**. È la firma di **`Z-251`** (la suite non regge la contesa sul DB condiviso), non un difetto del lavoro.
  - 📦 **NOTA S1053 — l'impianto che questa nota descrive è stato RITIRATO** (decisione di Enzo, → `#173`). La corsa presidiata, i lavoratori in parallelo, gli alberi di lavoro e l'istruttoria non esistono più: gli script citati qui sotto sono stati rimossi e i piani archiviati in `docs/archive/modalita-gov-ritirata/`. La chiusura resta valida — il criterio *fu* soddisfatto quando fu verificato — ma chi legge non deve andare a cercare comandi che non esistono più.
  - ✅ **CHIUSO S1052** (2026-08-10 09:00) — **il criterio è soddisfatto in tutte e tre le sue parti**: (a) `.zp/runs.ndjson` porta corse presidiate concluse, e una ha prodotto **un cluster chiuso con il suo commit** — `Z-112`, verdetto VERDE, merge `b6824e7e`; (b) i **4 test `[a mano]` sono stati eseguiti** — tre superati, il terzo (troncamento da budget) **eseguito ma NON probante**, e resta scritto così invece di essere spacciato per verde; (c) **Enzo ha deciso sul freno**: `meta.autorizzato_non_presidiato` resta **false**, e ora governa esattamente ciò che il suo nome dice — la corsia `full-presidiata` non ci passa, ogni altra sì.
  - **Cosa lascia aperto, dichiarato**: il *troncamento da budget* non è mai stato osservato (il tetto ha contenuto la spesa in tutte le corse — 10,96/12 · 5,28/12 · 0,95/1 · 9,75/12 — ma il taglio a metà lavoro no); e il protocollo dei tre revisori adversarial affida i verdetti a un workflow asincrono che sopravvive alla sessione, quindi un lavoratore che finisce il budget non può più leggerli. Entrambi vanno in coda come voci proprie, non nascosti dentro una casella spuntata.

  - nota-S1053: la «corsa presidiata» che ha chiuso questo cluster apparteneva alla modalita' gov, ritirata il 2026-08-10 (decisione di Enzo — piani e resoconto in `docs/archive/modalita-gov-ritirata/`). Il fatto resta storico e valido; l'impianto che lo produsse non esiste piu', e il loop e' tornato a una sessione sola.
...
- [x] **Z-112** (1.0h) — Nessun assert di drift post-suite (residui E2E% sul DB condiviso)
  ...
  - *assorbe*: `p100x:QW-F3`
  - ✅ **CHIUSO S1052** (2026-08-10) — **primo cluster chiuso da una corsa presidiata a più lavoratori**. Lavorato dal lavoratore 1 nel suo albero (commit `7eb39abf`: helper `drift-check.ts` 193 righe + test di integrazione 77 + aggancio in `vitest.config.ts`), istruito da gov con **verdetto VERDE** (`.zp/verdetti/w1-Z-112-20260810-090053.json` — file dell'era gov, ritirata S1053: il verdetto sopravvive in questa riga, non sul disco; **zero rilievi**: perimetro rispettato, typecheck e lint verdi, cancello test verde in 32 minuti, evidenze ammesse), portato in main col merge `b6824e7e`.
  - **Le due prove che ADR-0026 pretende, su livelli diversi**: `psql` sullo stato del database + `integration` sul sistema via API — la validazione lo dice con parole sue, «i punti ciechi non coincidono». Alla prima istruttoria ne aveva **una sola** e il verdetto fu **ROSSO**: il cancello ha respinto un lavoro tecnicamente completo perché le prove non bastavano, ed è servito un secondo giro del lavoratore per registrarla.
  - **Il lavoratore proponeva `cluster-interrupted`**, non la chiusura: il suo protocollo non ammette di chiudere senza i tre revisori adversarial, e il workflow che li ospitava era rimasto orfano di una sessione finita per budget. Il giudizio spetta a gov, che decide sui cancelli e sulle evidenze — non sull'autovalutazione di chi ha fatto il lavoro. **Il difetto resta aperto e va registrato**: un protocollo che affida i verdetti a un workflow asincrono è incompatibile con sessioni che finiscono per budget.
