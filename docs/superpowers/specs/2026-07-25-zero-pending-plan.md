...
- [ ] **Z-250** (6.0h) — **Rendere sicuro l'impianto `zero-pending-loop` prima di togliere il freno.** L'impianto è completo e versionato (skill + `scripts/zero-pending-driver.sh` + 6 tool `docs/kb/tools/zp_*.py`, commit `cb66c1e0`), ma `meta.autorizzato_non_presidiato: false` lo tiene fermo: il driver esce 3 senza aprire nulla. Due revisioni ostili (S1030) hanno chiuso i difetti che rendevano l'esecuzione pericolosa — lock non atomico e cancellato dal processo sbagliato, `kill` che non fermava il driver, deploy di produzione fuori da ogni filtro (ora veto in `close-propagate.sh`), tetto di spesa inerte, «zero pendenze» dichiarabile con righe di piano non lette, CI «verde» con zero workflow valutati, API di produzione ferma vista come sana, alert «accettati» per coincidenza lessicale. **Restano aperti i rilievi che chiedono un ridisegno**, e sono la condizione per togliere il freno:
  1. **classificazione sul criterio sbagliato** — la classe si deduce dalla descrizione e non dal *chiuso quando*, che è dove sta l'azione: in corsia non presidiata finiscono `Z-153` (deploy sul sito pubblico), `Z-070` (backfill ~14k skill sul DB reale), `Z-186` (import 139k archi), `Z-048` (PATCH reali su `/v1/me/*`), `Z-167`, `Z-108`, `Z-209`;
  2. **`class_c_preconditions` non è letto da alcuna riga di codice** — dump verificato <24h, prova su linux-pc, doppia esecuzione e diff `pg_dump` vuoto sono prosa;
  3. **il tipo di prova è autodichiarato** — due `echo` chiudono un cluster — e con output UTF-8 la prova viene registrata verde e **vuota** (i sottoprocessi ereditano cp1252 su Windows);
  4. **il gate delle prove contraddice la Definition of Done**: rifiuta `integration+e2e` e `psql+runtime`, che ADR-0026 e CLAUDE.md impongono, e accetta `staticcheck` come mezza prova;
  5. **`zp_selftest` non rileva 4 regressioni su 5** (effort, budget-ore, contatore fallimenti, marcatore di blocco su Enzo non sono coperti; i test girano su dati vivi e sono tautologici);
  6. scritture di stato non atomiche, id duplicato che sblocca le dipendenze, `--budget-ore` mai passato dal driver.
  - *chiuso quando*: `meta.autorizzato_non_presidiato: true` con i 6 punti chiusi, e una corsa reale in corsia safe che chiude almeno un cluster senza toccare la produzione (evidenza: `.zp/runs.ndjson` + diff del commit prodotto)
  - *nota*: l'autonomia non presidiata resta una **decisione di Enzo** — il design è ancora «BOZZA in attesa di approvazione». Il freno non si toglie senza il suo via, anche a rilievi chiusi
