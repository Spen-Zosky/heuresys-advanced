...
- [ ] **Z-250** (2.5h) — La skill `zero-pending-loop` è pronta come **istruzioni** ma non come **macchina**, e non è versionata. Presenti e completi: `SKILL.md`, i 10 reference (protocol, adversarial, blast-radius, gates, operations, close, bootstrap, selection, driver, LEARNINGS), `references/zp.config.yaml` e due file di eval. Assenti i tre pezzi eseguibili che la SKILL stessa dichiara come pre-condizione non negoziabile — `docs/kb/tools/zp_zero_check.py`, `docs/kb/tools/zp_gate.py`, `scripts/zero-pending-driver.sh` — e l'intera directory è **untracked** (`git ls-files` vuota). Il design (`2026-07-25-zero-pending-loop-design.md`) è ancora «BOZZA — in attesa di approvazione Enzo». La skill si auto-blocca correttamente quando i gate mancano, quindi non è pericolosa: è inerte
  - *chiuso quando*: i 3 file eseguibili esistono, `git ls-files .claude/skills/zero-pending-loop/` non è vuota, e un giro a vuoto del driver produce `.zp/last-outcome.json` valido — oppure la directory è rimossa e il design marcato non-implementato
  - *nota*: l'autonomia non presidiata che il design abilita è una decisione di Enzo, non tecnica
  - *rettifica S1030*: la prima stesura di questa voce diceva «2 reference su 9». Era un errore di misura mio — avevo troncato l'output di `ls -R` con `head -30` e letto la lista a metà. I reference ci sono tutti
