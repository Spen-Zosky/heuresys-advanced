...
**Header hygiene**: when cloning the scope-suite header comment, DROP the literal `(password <TEST_ADMIN_PASSWORD>)` fragment — stale pre-F-001 residue; write `(password env-driven, F-001)`. Flag the residue in the 10 existing files to Enzo in the report (cleanup candidate, not this mission).
...
- **L3 (LOW, applied)** — the gold-pattern header carries a stale `(password <TEST_ADMIN_PASSWORD>)` pre-F-001 fragment; M6 now forbids cloning it and flags the 10-file residue to Enzo in the report.
