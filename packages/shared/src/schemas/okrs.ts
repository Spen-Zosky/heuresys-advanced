...
export const OkrSchema = z.object({
  ...
  // `status` resta visibile per mandato (ADR-0032/I20); se ne va il QUANTO.
...
export const OkrKeyResultSchema = z.object({
  ...
  // `startValue` e `targetValue` RESTANO: sono la DEFINIZIONE del risultato
  // atteso, cioe' struttura. Se ne va dove la persona e' arrivata.
  startValue: z.number(),
  ...
  weight: z.number(),
  confidenceLevel: z.number().int().optional(),
  masked: z.array(z.string()).optional(),
  createdAt: z.iso.datetime(),
  updatedAt: z.iso.datetime(),
});
...
export const OkrCheckInSchema = z.object({
  ...
  overallProgress: z.number().nullable().optional(),
